...
{
    cout<<__FILE__<<":"<<__LINE__<<" PATH_MAX = " << PATH_MAX <<endl;
    char temp [ PATH_MAX ];
	...
	if (currentDir.compare("/") == 0) {
	    cout<<__FILE__<<":"<<__LINE__<<" Current directory is : "<<currentDir<<" . This is unacceptable! We are changing to your home directory."<<std::endl;
	    char* myHome = getenv("HOME");
	    int chdirError = chdir (myHome);
	    if (chdirError) {
		    ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" Unable to chdir to "<<myHome<<endl; 
	    ...
	    if (GetCurrentDir(temp, PATH_MAX) != 0) {
            ...
            cout<<__FILE__<<":"<<__LINE__<<" Current directory is : "<<currentDir<<" ."<<std::endl;
	    ...
	    if (chdirError) {
		    ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" Unable to chdir to "<<newPath<<endl; 
		    ErrorManager::instance.treatError();
	    }
	    if (GetCurrentDir(temp, PATH_MAX) != 0) {
	    currentDir = String (temp); 
...

{
    ...
    cout<<__FILE__<<":"<<__LINE__<<" : Applying "<< atomSpringContainer.numAtomSprings()<<" atomSpring's."<<endl;
    for (int i = 0; i < atomSpringContainer.numAtomSprings() ; i++)
    {
        ...
        // cout<<__FILE__<<":"<<__LINE__<<" : Applying atom spring : "<<myAtomSpring.atom1Chain<<" "<<myAtomSpring.atom1Residue.outString()<<" " <<myAtomSpring.atom1Name<<" "<<myAtomSpring.atom2Chain<<" "<<myAtomSpring.atom2Residue.outString()<<" " <<myAtomSpring.atom2Name<<endl;
        ...
        } else if (waterDropletContainer.hasChainID(myAtomSpring.atom1Chain ))
        {
            ...
            if (myAtomSpring.atom1Name.compare("OW") != 0) {    
                ErrorManager::instance <<__FILE__<<":"<<__LINE__<<"Error! For water, atoms other than OW are not currently supported. You specified "<< myAtomSpring.atom1Name<<endl; 
                ErrorManager::instance.treatError();
            }
            ...
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<"Error! You have tried to attach a spring or tether to atom "<<myAtomSpring.atom1Name<<" of chain "<<myAtomSpring.atom1Chain<<". It does not exist!"<<endl; 
            ErrorManager::instance.treatError();
        }
        ...
        }  else if (myMonoAtomsContainer.hasChainID(myAtomSpring.atom2Chain))
        {
            ...
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<"Error! You have tried to attach a spring or tether to atom "<<myAtomSpring.atom2Name<<" of chain "<<myAtomSpring.atom2Chain<<". It does not exist!"<<endl; 
            ErrorManager::instance.treatError();
        }
        ...
        if (  myAtomSpring.tether  )             {
            ...
            if (verbose) cout<<__FILE__<<":"<<__LINE__<<" groundLocation1 = "<<groundLocation1<<endl;
            if (verbose) cout<<__FILE__<<":"<<__LINE__<<" groundLocation2 = "<<groundLocation2<<endl;
            ...
            if (verbose) cout<<__FILE__<<":"<<__LINE__<<" myAtomSpring.deadLengthFraction = "<<myAtomSpring.deadLengthFraction<<endl;
            if (verbose) cout<<__FILE__<<":"<<__LINE__<<" originalLength = "<<originalLength<<endl;
            if (verbose) cout<<__FILE__<<":"<<__LINE__<<" myAtomSpring.deadLength = "<<myAtomSpring.deadLength<<endl;
            ...
            // cout<<__FILE__<<":"<<__LINE__<<": Adding a two point linear spring with location 1 = "<<location1<<" and location 2 = "<<location2<< endl;
        }
    }
    cout<<__FILE__<<":"<<__LINE__<<endl;
...
double   ParameterReader::myAtoF(  map<const String,double> myUserVariables,  const char* value){
    cout<<__FILE__<<":"<<__LINE__<<" inside myAtoF. converting string : >"<<value<<"<"<<endl; 
    ...
    if (plusPosition != String::npos) { // We have a '+' to deal with
        ...
        //cout<<__FILE__<<":"<<__LINE__<<" The increment String is : "<<incrementString<<endl;
        ...
        if (minusPosition== 0) {
            ...
            cout<<__FILE__<<":"<<__LINE__<<" Detected a leading \'-\' sign. Will insert a zero to the left of the \'-\'."<<endl;
        ...
        cout<<__FILE__<<":"<<__LINE__<<" baseDoubleString =  >"<<baseDoubleString  <<"< "<<endl;
        ...
        cout<<__FILE__<<":"<<__LINE__<<" About to extract numerical decrement from the string >"<<decrementString<<"< "<<endl;
        ...
        if (!((increment == -1111 ) && (decrement == -1111 )  )) {
            cerr <<__FILE__<<":"<<__LINE__<<" Unexplained error!"<<endl;
            exit(EXIT_FAILURE);
        }
        baseDoubleString = String(value);        
        //if (!(isNumber(baseDoubleString))) {
	//    ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" Error! Expected a number, got >"<<baseDoubleString<<"< "<<endl;
   	//    ErrorManager::instance.treatError();
        //};
        increment = 0;
        decrement = 0;
        double baseDouble;
        {
            ...
            if ((baseDoubleString.substr(0,1)).compare("@") ==0) {
                ...
                {
                    cerr <<__FILE__<<":"<<__LINE__<<": Undefined user variable "<<value<<endl;
                    exit(EXIT_FAILURE);
                }

                cout<<__FILE__<<":"<<__LINE__<<" Read user variable "<<baseDoubleString.c_str()<<"  which is set to : "<<myUserVariables[baseDoubleString.c_str()]<<endl;
        ...
        if (isFixed(baseDoubleString)) {
                    baseDouble = (atof(baseDoubleString.c_str()));
                        cout<<__FILE__<<":"<<__LINE__<<" We appear to be in a leaf of the recursion tree for myAtoF. Parsed >"<<baseDoubleString<< "< as : "<<baseDouble<<endl;
        } else {
            // Actually we should not get to this point. isFixed itself has plenty of ErrorManager calls for funky errors.
                        ErrorManager::instance <<__FILE__<<":"<<__LINE__<<"There was an error processing a putative floating point number."<<endl;
                        ErrorManager::instance.treatError();
        }
            }
        }
    ...
    cout<<__FILE__<<":"<<__LINE__<<" : Result of >"<< value  <<"< is : " << finalDouble <<endl;
    return finalDouble;
}
                ...
                ((baseOperationVector[i].FirstBPEdge).compare("Rigid"    ) == 0)) 
        {
            ...
            for (int j = 0 ; j< (int)basePairContainer.numBasePairs() ; j++) {
                ...
                if ( 
                    ...
                    //cout<<__FILE__<<":"<<__LINE__<<" about to delete BASE PAIR " <<j<<endl; 
...
void ParameterReader::printAllSettings (ostream & myOstream, String remarkString ) { 
    ...
    myOstream << remarkString << "removeMomentumPeriod                   double   "<<removeRigidBodyMomentum<<endl;
...
void ParameterReader::removeNonPriorityBasePairs (int priorityLevel) {  
    ...
    //ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : base pair priorities are being phased out!"<<endl;
    //ErrorManager::instance.treatError();
    int oldDutyCyclePriority=0;             
    priority = priorityLevel; //not sure if this is such a great place to set this parameter.

    cout <<__FILE__<<":"<<__LINE__<<" : removeNonPriorityBasePairs] At this stage, temperature = "<< temperature  <<endl;
...
void ParameterReader::updateAllResiduesWithin(int index, String chainID, int resID, double radius){
    ...
    if(index < 0 || index >= includeAllResiduesWithinVector.size()){
        ErrorManager::instance <<__FILE__<<":"<<__LINE__<<": you tried to update a non existing includeAllResiduesWithin command." << endl;
        ErrorManager::instance.treatError();
    }
...
void ParameterReader::deleteAllResiduesWithin(int index){
    if(index < 0 || index >= includeAllResiduesWithinVector.size()){
        ErrorManager::instance <<__FILE__<<":"<<__LINE__<<": you tried to delete a non existing includeAllResiduesWithin command." << endl;
        ErrorManager::instance.treatError();
    }
                                                            ...
                                                            int resID)
{
    ...
    if(index < 0 || index >=  physicsContainer.getNumResidueStretches()){  // includeAllNonBondAtomsInResidueVector.size()){
        ErrorManager::instance <<__FILE__<<":"<<__LINE__<<": you tried to update a non existing includeAllNonBondAtomsInResidue command." << endl;
        ErrorManager::instance.treatError();
    }
                                                 ...
                                                 const bool readExcept)
{
       ...
       )   
    {
        if (densityContainer.numDensityStretches() > 0) {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You have already declared "<<densityContainer.numDensityStretches()<<" biopolymer stretches to be fitted to the density map.  Please do this after you have created ALL biopolymer chains."<<endl; ErrorManager::instance.treatError();
        }
        ...
        for (int i = 0 ; i < (int)waterDropletAboutResidueVector.size(); i++) {
            if (waterDropletAboutResidueVector[i].waterDropletChainID.compare(parameterStringClass.getString(1) ) == 0) {    
                ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : Can't use chain "<<parameterStringClass.getString(1)<<" to identify this "<<parameterStringClass.getString(0)<<", because it is already being used by a water droplet."<<endl;
                ErrorManager::instance.treatError();
            }
        }
        if (
        ...
        { 
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You must specify any general sterics command (e.g. addAllHeavyAtomSterics, addProteinBackboneSterics, addSelectedAtoms, addProteinBackboneSterics, addRNABackboneSterics) AFTER the last biopolymer is specified.  "<<endl;
            ErrorManager::instance.treatError();

        }

        if (contactContainer.numContacts() > 0) {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You can only issue a contact command AFTER the last biopolymer is specified.  "<<endl;
            ErrorManager::instance.treatError();
        }
        ...
        if (parameterStringClass.getString(4).length() >0){
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You have provided too many parameters when specifying the sequence! You should use, for example :  \"RNA A 16 AUGC \" .. this creates an RNA with chain ID \"A\", first residue number = 16, and sequence AUGC."<<endl;
            ErrorManager::instance.treatError();
        }
        if (((parameterStringClass.getString(0)).compare("proteinSequence") ==0 ) || ((parameterStringClass.getString(0)).compare("protein") ==0 ) || ((parameterStringClass.getString(0)).compare("Protein") ==0 )  ) {
            ...
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<": Unsupported sequence type."<<endl;    
            ErrorManager::instance.treatError();
        ...
        {
            //ErrorManager::instance <<__FILE__<<":"<<__LINE__<<": Unsupported sequence type: DNA"<<endl;  
            //ErrorManager::instance.treatError();
            myBiopolymerClassContainer.addBiopolymerClass(parameterStringClass.getString(3),parameterStringClass.getString(1), 
                    ResidueID(parameterStringClass.getString(2)) ,
    ...
    tempArray[2] = "34"  ; 
        ...
        moleculeClassContainer.add(myChain,myResidueName,myMoleculeClass);
            //ErrorManager::instance <<__FILE__<<":"<<__LINE__<<":  this command is not ready yet."<<endl;
            //ErrorManager::instance.treatError();
        return;
    ...
    if  ((parameterStringClass.getString(0)) == (  "renumberBiopolymerResidues"))  {
        cout<<endl<<__FILE__<<":"<<__LINE__<<": This command renumbers biopolymers. "<<endl;//New residues numbers will be consecutively increasing integers starting with that provided. "<<endl; 
        cout<<__FILE__<<":"<<__LINE__<<": Syntax:  "<<endl; 
        cout<<__FILE__<<":"<<__LINE__<<": renumberBiopolymerResidues                                     : This renumbers all biopolymers to start with residue number 1."<<endl; 
        //cout<<__FILE__<<":"<<__LINE__<<": renumberBiopolymerResidues <first residue number> >          : This renumbers all biopolymers to start with the specified residue number."<<endl;
        //cout<<__FILE__<<":"<<__LINE__<<": renumberBiopolymerResidues <chain ID> <first residue number> : This renumbers the specified biopolymers to start with the specified residue number."<<endl<<endl;

        if (parameterStringClass.getString(1).length() != 0)  { ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : Too many parameters for this command."<<endl; ErrorManager::instance.treatError();}
        ...
        {
            //ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : Not enough parameters for this command."<<endl; ErrorManager::instance.treatError();
            cout<<__FILE__<<":"<<__LINE__<<": You have called renumberBiopolymerResidues with no parameters. We interpret this to mean that you want to renumber all biopolymers to start with 1."<<endl;
        ...
        } else if (parameterStringClass.getString(2).length() == 0) 
        {
            ...
            cout<<__FILE__<<":"<<__LINE__<<": You have called renumberBiopolymerResidues with one parameter. We interpret this to mean that you want to renumber all biopolymers to start with the residue number : "<< parameterStringClass.getString(1)  <<endl;
    ...
    {
        cout<<__FILE__<<":"<<__LINE__<<": This command sets the default backbone (for proteins: psi, phi, and peptide bond angles) for the specified chain and range of residues, to those of the specified secondary structure.  For secondary structure type \'Alpha\' (meaning alpha helical) that is -60 degrees for both. Note that these are the DEFAULT dihedrals, if there is any structure in the input file for any of the given residues, that structure prevails. "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<": At the moment only these backbone configurations are supported: Alpha, ParallelBeta, and AntiParallelBeta.  These work only on proteins, of course. "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<": Note also that you cannot apply this to the first or last residue on the chain, because the phi and psi are defined using atoms from preceding and succeeding residues. "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<": setBackboneAngles <chain ID> <start residue number> <end residue number> <Alpha | ParallelBeta | AntiParallelBeta>."<<endl;

        if ((parameterStringClass.getString(4).length() == 0) ) { ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : Not enough parameters for this command."<<endl; ErrorManager::instance.treatError();}
        if ((parameterStringClass.getString(5).length() != 0) ) { ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : Too many parameters for this command."<<endl; ErrorManager::instance.treatError();}
        ...
        cout<<__FILE__<<":"<<__LINE__<<": Start residue identified as : "<<startResidue.outString()<<endl; 
        cout<<__FILE__<<":"<<__LINE__<<": End residue identified as : "<<endResidue.outString()<<endl; 
    ...
    if  (((parameterStringClass.getString(0)).compare(  "substituteResidue") == 0))  {
        cout<<__FILE__<<":"<<__LINE__<<": This command generates a specified substitution in a specified chain, at a specified residue position. Syntax: substituteResidue <chain ID> <residue number> <new residue type> ."<<endl;
    ...
    if  (((parameterStringClass.getString(0)).compare(  "insertResidue") == 0))  {
        cout<<__FILE__<<":"<<__LINE__<<": This command generates a specified insertion in a specified chain."<<endl;
        cout<<__FILE__<<":"<<__LINE__<<": Syntax: insertResidue <chain ID> <residue number> <new residue type> ."<<endl;
        if (parameterStringClass.getString(4).size() != 0) {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<": You have specified too many parameters for this command."<<endl;
            ErrorManager::instance.treatError();
        }
        ...
        cout<<__FILE__<<":"<<__LINE__<<": This command deletes the specified residue in the specified chain. The remaining residues retain the PDB residue numbers they had prior to the deletion (i.e. there may result a gap in the numbering)."<<endl;
            cout<<__FILE__<<":"<<__LINE__<<": Syntax: deleteResidues <chain ID> <residue ID>  "<<endl;
            cout<<__FILE__<<":"<<__LINE__<<": Syntax: deleteResidues <chain ID> <start residue ID> <end residue ID>  "<<endl;
            cout<<__FILE__<<":"<<__LINE__<<": Or, you could just delete the entire chain, like this: "<<endl;
            cout<<__FILE__<<":"<<__LINE__<<": Syntax: deleteResidues <chain ID>   "<<endl;
            cout<<__FILE__<<":"<<__LINE__<<": Or, you could delete ALL chains, like this: "<<endl;
            cout<<__FILE__<<":"<<__LINE__<<": Syntax: deleteResidues    "<<endl;
            cout<<__FILE__<<":"<<__LINE__<<": But be careful with this command! Make sure you don't try to reference any residues that this command deletes, elsewhere in your command file."<<endl;
    
        String myChain = parameterStringClass.getString(1);
            if (safeParameters) if (!mobilizerContainer.isEmpty()) {
                ErrorManager::instance <<__FILE__<<":"<<__LINE__<<": You have previously called some sort of mobilizer command. Please move all such commands, which reference residue numbers, below the deleteResidues command. Otherwise there  too much potential to reference a deleted residue. You can override this message by setting safeParameters False"<<endl;
                ErrorManager::instance.treatError();
            }
            if (parameterStringClass.getString(1).size() == 0) {
                cout<<__FILE__<<":"<<__LINE__<<": It appears you wish to delete ALL chains "<<endl;
                ...
                cout<<__FILE__<<":"<<__LINE__<<": It would appears you wish to delete chain "<<myChain<<endl;
                myBiopolymerClassContainer.deleteBiopolymerClass(myChain);
                if (myBiopolymerClassContainer.hasChainID(myChain)) {
                    ErrorManager::instance <<__FILE__<<":"<<__LINE__<<": Error! For some reason chain "<<myChain<<" still exists!"<<endl;
                    ErrorManager::instance.treatError();
                }
                else cout<<__FILE__<<":"<<__LINE__<<": Done! the chain "<<myChain<<" is gone!"<<endl;
                return;
            }
            ...
            if (parameterStringClass.getString(3).size() != 0) {
                cout<<__FILE__<<":"<<__LINE__<<" You have specified a range of residues.."<<endl;
            ...
            if ((startResidue == myBiopolymerClassContainer.updBiopolymerClass(myChain).getFirstResidueID()) &&
                (endResidue == myBiopolymerClassContainer.updBiopolymerClass(myChain).getLastResidueID())) {
                    ErrorManager::instance <<__FILE__<<":"<<__LINE__<<": You can't delete ALL residues in the chain this way! Just delete the entire chain. See syntax above."<<endl;
                    ErrorManager::instance.treatError();
            }
            ...
            while (deletedResidue<=endResidue) {
                if (myBiopolymerClassContainer.updBiopolymerClass(myChain).getLastResidueID() == myBiopolymerClassContainer.updBiopolymerClass(myChain).getFirstResidueID()) {
                    ErrorManager::instance <<__FILE__<<":"<<__LINE__<<": You can't delete ALL residues in the chain!  not with this command, anyway."<<endl;
                    ErrorManager::instance.treatError();
                }
                cout<<__FILE__<<":"<<__LINE__<<" About to delete "<<deletedResidue.outString()<<endl;
        ...
        if  (((parameterStringClass.getString(0)).compare("renameChain") == 0))  {
            ...
            cout<<__FILE__<<":"<<__LINE__<<": Syntax:"<<endl;
            cout<<__FILE__<<":"<<__LINE__<<": renameChain <old chain ID> <new chain ID>"<<endl;
            cout<<__FILE__<<":"<<__LINE__<<": Works only with biopolymers for now.     "<<endl;
            parameterStringClass.validateNumFields(3);
            ...
            if (!(myBiopolymerClassContainer.hasChainID(oldChainID))) {
                ErrorManager::instance <<__FILE__<<":"<<__LINE__<<": Error! There is no biopolymer with chain >"<< oldChainID  <<"< !"<<endl;
                ErrorManager::instance.treatError();
            }    
            ...
            if ((myBiopolymerClassContainer.hasChainID(oldChainID))) {
                ErrorManager::instance <<__FILE__<<":"<<__LINE__<<": Error! There is still a biopolymer with old chain >"<< oldChainID  <<"< !"<<endl;
                ErrorManager::instance.treatError();
            }    
            if (!(myBiopolymerClassContainer.hasChainID(newChainID))) {
                ErrorManager::instance <<__FILE__<<":"<<__LINE__<<": Error! There is no biopolymer with chain >"<< newChainID  <<"< !"<<endl;  
                ErrorManager::instance.treatError();
            }    
            return;
        }    
        ...
        if  (((parameterStringClass.getString(0)).compare(  "monoAtoms") == 0))  {
            ...
            cout<<__FILE__<<":"<<__LINE__<<": Syntax:"<<endl;
            cout<<__FILE__<<":"<<__LINE__<<": monoAtoms <chain ID> <first residue number>   <number of ions> <atom name>"<<endl;
    ...
    {
        cout<<__FILE__<<":"<<__LINE__<<": This command allows to remove a chain from MMB. The chain will not be present in the output PDB files." << endl;
        cout<<__FILE__<<":"<<__LINE__<<": Syntax: deleteChain <chainID>" << endl;
    ...
    if ( ((parameterStringClass.getString(0)).compare("atomSpring")      == 0) ||
        ...
        {myAtomSpring.tether = true ;
            ...
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<"Error!"<<endl;
            ErrorManager::instance.treatError();
        }
        cout << __FILE__<<":"<<__LINE__<< "Note that MMB and molmodel use units of nm, kJ/mol, ps, and daltons (g/mol). In prior releases of MMB, including 2.10, length was taken in Å -- however we are going back to nm length units, with apologies for the confusion.  "<<endl;
        cout << __FILE__<<":"<<__LINE__<< "For reference, Amber99 uses Å, kcal/mol, and ps.  So for example a carbon-carbon bond stretch force constant is 310 kcal/mol/Å/Å, so in our units would be 310*4.1868*10*10 =129790.8 kJ/mol/nm/nm"<<endl;
                ...
                ((parameterStringClass.getString(0)).compare("atomTether")      == 0))
        {
            ...
            if (myBiopolymerClassContainer.hasChainID(myAtomSpring.atom1Chain)) {
                ...
                ErrorManager::instance <<__FILE__<<":"<<__LINE__<<"Error! No biopolymer or monoAtoms container has this chain ID : >" <<myAtomSpring.atom1Chain<<"<"<<  endl;
                ErrorManager::instance.treatError();
            }
            ...
            /*if ((!(myBiopolymerClassContainer.hasChainID(myAtomSpring.atom1Chain))) &&
                ...
                ErrorManager::instance << __FILE__<<":"<<__LINE__<<" Could not find chain "<<myAtomSpring.atom1Chain<<endl; ErrorManager::instance.treatError();
            }*/
            ...
            if (myBiopolymerClassContainer.hasChainID(myAtomSpring.atom2Chain)) {
                ...
                ErrorManager::instance <<__FILE__<<":"<<__LINE__<<"Error! No biopolymer or monoAtoms container has this chain ID : >" <<myAtomSpring.atom2Chain<<"<"<<  endl;
                ErrorManager::instance.treatError();
            }
            ...
            /*if ((!(myBiopolymerClassContainer.hasChainID(myAtomSpring.atom2Chain))) &&
                ...
                ErrorManager::instance << __FILE__<<":"<<__LINE__<<" Could not find chain "<<myAtomSpring.atom2Chain<<endl; ErrorManager::instance.treatError();
            }*/
            ...
            if ((parameterStringClass.getString(7)).length() >0) {
                ...
                cout<< __FILE__<<":"<<__LINE__<<" Setting dead length to : "<<myAtomSpring.deadLength<<" nm"<<endl;
            ...
            if ((parameterStringClass.getString(8)).length() >0) {
                ...
                cout<< __FILE__<<":"<<__LINE__<<" Setting force Constant to : "<<myAtomSpring.forceConstant<<" kJ/mol/nm/nm"<<endl;
            } else
        ...
        if (((parameterStringClass.getString(0)).compare("springToGround") == 0) || 
                ((parameterStringClass.getString(0)).compare("tetherToGround") == 0)) {
            cout<< __FILE__<<":"<<__LINE__<<"format:  springToGround atom1Chain atom1Residue atom1Name  groundX groundY groundZ       deadLength forceConstant"<<endl;
            myAtomSpring.atom1Chain   = parameterStringClass.getString(1);
            if (myBiopolymerClassContainer.hasChainID(myAtomSpring.atom1Chain)) {
                ...
                ErrorManager::instance << __FILE__<<":"<<__LINE__<<" There is no biopolymer or monoAtom which has the specified chain. "<<endl; ErrorManager::instance.treatError();
            }
            myAtomSpring.atom1Name    = parameterStringClass.getString(3);
            if ((!(myBiopolymerClassContainer.hasChainID(myAtomSpring.atom1Chain))) &&
                    (!(myMonoAtomsContainer.hasChainID(myAtomSpring.atom1Chain))) &&
            ...
            cout<< __FILE__<<":"<<__LINE__<<" Setting ground location to : "<<myAtomSpring.groundLocation<<" nm, nm, nm "<<endl;
            cout<< __FILE__<<":"<<__LINE__<<" In prior releases of MMB we took locations in Å.  We are going back to nm, kJ/mol, and ps for consistency with molmodel, with apologies for the confusion. "<<endl;
            myAtomSpring.toGround     = true ;
            if ((parameterStringClass.getString(7)).length() >0) {
                myAtomSpring.deadLength   = myAtoF(userVariables,parameterStringClass.getString(7).c_str());
                cout<< __FILE__<<":"<<__LINE__<<" Setting dead length to : "<<myAtomSpring.deadLength<<" nm"<<endl;
                cout<< __FILE__<<":"<<__LINE__<<"Here again, we are going back to nm whereas before we took Å, again with apologies for the confusion. "<<endl;
            }
            else myAtomSpring.deadLength   = 0.0;
    ...
    if ( ((parameterStringClass.getString(0)).compare("nucleicAcidDuplex")      == 0) ) 
    {
        ...
        cout<<__FILE__<<":"<<__LINE__<<" : format: nucleicAcidDuplex <Chain ID A> <start residue A> <end residue A> <Chain B ID> <start residue B> <end residue B> " <<  endl; 
        //cout<<__FILE__<<":"<<__LINE__<<" : format:  nucleicAcidDuplex <Chain ID A> <start residue A> <end residue A> <Chain B ID> <start residue B> <end residue B> "<<std::cout;
        cout<<__FILE__<<":"<<__LINE__<<" : where start residue A will be Watson-Crick base paired with  end residue B, etc.  we require: (start residue A) <= (end residue A) and (start residue B) >= (end residue B), because duplexes are antiparallel."<<endl;
        ...
        if (parameterStringClass.getString(7).length() >0) {
            cout<<__FILE__<<":"<<__LINE__<<" : You have specified too many parameters for this command.  Correct format is: "<<endl<<"nucleicAcidDuplex <Chain ID A> <start residue A> <end residue A> <Chain B ID> <start residue B> <end residue B> "<<endl;
        ...
        if (!(parameterStringClass.getString(6).length() >0)) {
            ...
            cout<<__FILE__<<":"<<__LINE__<<" : You have not specified enough parameters for this command.  Correct format is: "<<endl<<"nucleicAcidDuplex <Chain ID A> <start residue A> <end residue A> <Chain B ID> <start residue B> <end residue B> "<<endl;
            ErrorManager::instance <<"Note that start Residue A <= end Residue A and start Residue B >= end Residue B, since duplexes are antiparallel."<<endl;
            ErrorManager::instance.treatError();
        ...
        {
            ErrorManager::instance << __FILE__<<":"<<__LINE__<<  "In the nucleicAcidDuplex command, the end residue must be greater than or equal to the start residue for each chain." << endl;
            ErrorManager::instance.treatError();
        }
        if(startResidueB < endResidueB)
        {
            ErrorManager::instance << __FILE__<<":"<<__LINE__<<  "In the nucleicAcidDuplex command, the start residue must be greater than or equal to the end residue for each chain." << endl;
            ErrorManager::instance.treatError();
        }
        ...
        {
            ErrorManager::instance << __FILE__<<":"<<__LINE__<< "In the nucleicAcidDuplex command, the two paired segments must be of the same length." << endl;
            ErrorManager::instance.treatError();
        }
     ...
     {
         cout<<__FILE__<<":"<<__LINE__<< " You have called the alignmentForces command. This applies springs pulling together two user-defined stretches of biopolymer residues. The springs go between corresponding atoms in corresponding residues. You may provide aligned stretches of residues, or let MMB figure out the gapped alignment for you with Seqan. It uses a simple scoring function, which differentiates between \"match\" (residues the same), \"mismatch\" (residues not the same), gap opening, and gap extension."<<endl;
         cout<<__FILE__<<":"<<__LINE__<< " Usage: ";

         cout<<__FILE__<<":"<<__LINE__<< " First consider any parameters you wish to change : "<<endl;
         cout<<__FILE__<<":"<<__LINE__<<" alignmentForces noGap : This sets the internal parameter alignmentForcesIsGapped to False, introduces a very high penalty for gaps in the alignment, and requires that the aligned fragments have the same number of residues. If you were looking for the old \"threading\" command, set this parameter."<<endl;
         cout<<__FILE__<<":"<<__LINE__<<" alignmentForces gapped : This means that gappedAlignment is left at True, and Seqan is used to figure out the alignment. This is the default behavior. "<<endl;
         cout<<__FILE__<<":"<<__LINE__<<" alignmentForces deadLengthFraction <fraction> : This sets the dead length of the springs to <fraction> * (initial length). Use this e.g. to do progressive morphing.  "<<endl;
         cout<<__FILE__<<":"<<__LINE__<<"                                                 For <fraction> in the interval (0, 1], this sets the dead length of the springs to <fraction> * (initial length). It also sets alignmentForcesDeadLengthIsFractionOfInitialLength = True. Set this to zero to set alignmentForcesDeadLengthIsFractionOfInitialLength = False and recover ordinary behavior. "<<endl;
         cout<<__FILE__<<":"<<__LINE__<<" alignmentForces forceConstant <force constant (double)> : This sets the force constant to the specified value. Unless you are changing it now, it will be : "<< alignmentForcesForceConstant<<" ."<<endl;

         cout<<__FILE__<<":"<<__LINE__<<endl;
         cout<<__FILE__<<":"<<__LINE__<< " As you may know, MMB command files are read from top to bottom, so the last parameter setting prior to the execution command prevails. Any parameters set an the execution command do not apply to that execution command. Now issue the execution command: "<<endl;

         cout<<__FILE__<<":"<<__LINE__<<" Syntax:  <alignmentForces> <Chain A>  <Chain-B> "<<endl;
         cout<<__FILE__<<":"<<__LINE__<<" Or    :  <alignmentForces> <Chain A>  <start residue A> <end residue A>  <Chain-B>  <start residue B> <end residue B>   "<<endl;
         cout<<__FILE__<<":"<<__LINE__<<endl;
         ...
         if (verbose) cout<<__FILE__<<":"<<__LINE__<< " Currently alignmentForcesIsGapped = "<<alignmentForcesIsGapped<<std::endl;
         if (verbose) cout<<__FILE__<<":"<<__LINE__<< " Currently alignmentForcesGapPenalty = "<<alignmentForcesGapPenalty<<std::endl;
         if (verbose) cout<<__FILE__<<":"<<__LINE__<< " Currently thread.isGapped = "<<thread.isGapped<<std::endl;
	 ...
	 double myForceConstant = 1000000000.0; // set to super high value just to make sure it's being reset later.
         ...
         if ((parameterStringClass.getString(2)).length()==0) // User intends to set a boolean  parameter
         {
             if ((parameterStringClass.getString(1)).compare("noGap")==0){
                    ...
                    cout<<__FILE__<<":"<<__LINE__<<"  You have set alignmentForcesIsGapped = "<<alignmentForcesIsGapped<<std::endl;
                    cout<<__FILE__<<":"<<__LINE__<<"  You have set alignmentForcesGapPenalty = "<<alignmentForcesGapPenalty<<std::endl;
                  ...
                  ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : Syntax error!"<<endl;
                  ErrorManager::instance.treatError();
             }
         ...
         if ((parameterStringClass.getString(1)).compare("deadLengthFraction")==0){
                    ...
                    if ((alignmentForcesDeadLengthFraction < 0.0 ) || (alignmentForcesDeadLengthFraction > 1.0 )){
                      if (safeParameters) {
             ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : alignmentForcesDeadLengthFraction should lie in the interval (0, 1] if you want to apply progressive forces, or be set to 0 if you want to restore normal behavior. "<<endl;
             ErrorManager::instance.treatError();
                         }
                         alignmentForcesDeadLengthIsFractionOfInitialLength = true;
                         alignmentForcesDeadLengthFraction = myAtoF(userVariables,parameterStringClass.getString(2).c_str());
                         cout<<__FILE__<<":"<<__LINE__<<"  You have set alignmentForcesDeadLengthIsFractionOfInitialLength = "<<alignmentForcesDeadLengthIsFractionOfInitialLength<<std::endl;
                         cout<<__FILE__<<":"<<__LINE__<< " You have set alignmentForcesDeadLengthFraction = " << alignmentForcesDeadLengthFraction <<std::endl;
                        return;
            ...
            if (alignmentForcesForceConstant <= 0.0) {
                ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" forceConstant must be greater than zero! You have specified : "<<alignmentForcesForceConstant<<endl;
                ErrorManager::instance.treatError();
            }
            return;
        }
            ...
            cout<<__FILE__<<":"<<__LINE__<< " Detected you wish to align chains "<< parameterStringClass.getString(1)<<" and "<<parameterStringClass.getString(2)<<endl;
	    String      chainA = parameterStringClass.getString(1);
	    String      chainB = parameterStringClass.getString(2);
            //cout<<__FILE__<<":"<<__LINE__<< " Detected you wish to align chains "<<chainA <<" and "<<chainB <<endl;

	    if ((parameterStringClass.getString(3)).length() >0) 
	    { 
		ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : Too many parameters for this command! This command no longer takes an optional <force constant> parameter, if that is what you were trying to provide. Instead set the force constant using syntax:  alignmentForces forceConstant <force constant (double)> "<<endl;
		ErrorManager::instance.treatError();
	    ...
	    if ((parameterStringClass.getString(4)).length() >0) 
	    {
		ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : Too many parameters for this command!"<<endl;
		ErrorManager::instance.treatError();
	    }
	     
	    cout << "READER ThreadForceConstant " << myForceConstant << endl;
    
            cout << __FILE__<<":"<<__LINE__<<" : Creating a gapped threading for chain >"<<chainA<<"< versus chain >"<<chainB<<"< with force constant : "<<myForceConstant<<endl;
            ...
            if (safeParameters) if (thread.isGapped == false) {
                ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : Error!  If you set \"noGap\" then this command requires residue numbers. Otherwise use a \"gapped\" alignment. "<<endl;
                 ErrorManager::instance.treatError();
            }
		 ...
		 ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : alignmentForces : parameter "<< parameterStringClass.getString(1)<<" with value "<<parameterStringClass.getString(2)<<" not recognized."<<endl;
		 ErrorManager::instance.treatError();
        }
        }// if ((parameterStringClass.getString(3)).length()==0) 
	    ...
	    ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : Wrong number of parameters for this command! This command no longer takes an optional <force constant> parameter, if that is what you were trying to provide. Instead set the force constant using syntax:  alignmentForces forceConstant <force constant (double)> "<<endl;
	    ErrorManager::instance.treatError();
		...
		//myForceConstant= myAtoF(userVariables,parameterStringClass.getString(7).c_str());
                //cout<<__FILE__<<":"<<__LINE__<<" myForceConstant= "<<myForceConstant<<endl;
	    }
	    //else
	    //{
	    //	myForceConstant= defaultForceConstant;
            //    cout<<__FILE__<<":"<<__LINE__<<" myForceConstant= "<<myForceConstant<<endl;
	    //}
            cout<<__FILE__<<":"<<__LINE__<<"alignmentForcesForceConstant = "<<alignmentForcesForceConstant<<endl;
...
//residueStart1   = myBiopolymerClassContainer.residueID(userVariables, parameterStringClass.getString(2),thread.chainID1);
	    ...
	    ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : Error! Please check your syntax!"<<endl;
	    ErrorManager::instance.treatError();
        }
    ...
    {
        ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : Obsolete command! Please use alignmentForces. Issue with no parameters to get syntax information."<<endl;
        ErrorManager::instance.treatError();
        return;
    }   //End Threading
    ...
    if ( ((parameterStringClass.getString(0)).compare("RNAThreading")      == 0) || 
        ...
        ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : This command is being phased out! Try the \"threading\" command."<<endl;
        ErrorManager::instance.treatError();
        String      chainA = parameterStringClass.getString(1);
        String      chainB = parameterStringClass.getString(4);
    ...
    if ( ((parameterStringClass.getString(0)).compare( "psiPhiMobility") == 0)) 
    {
        ...
        ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You have specified too many parameters for this command. "<<endl ; ErrorManager::instance.treatError();
        } // Catch error
        ...
        {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You have specified an incorrect number of parameters. "<<endl ;
            ErrorManager::instance.treatError();
        }
            ...
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You have specified an incorrect number of parameters. "<<endl ; ErrorManager::instance.treatError();
        }
        return;
    } //End psiPhiMobility

    if ( ((parameterStringClass.getString(0)).compare( "doubleHelix") == 0)) {
        cout<<__FILE__<<":"<<__LINE__<<"  Syntax: doubleHelix <chain A> <lowest numbered residue chain A> <highest numbered residue chain A> <chain B> <highest numbered residue chain B> <lowest numbered residue chain B> "<<endl;
        ...
        if (higherA <= myBiopolymerClassContainer.updBiopolymerClass(parameterStringClass.getString(1)).sum(lowerA,1)) {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" for the first chain, specify first the lower numbered, then the higher numbered residue. Helix must be at least 3BP long."<<endl;
            ErrorManager::instance.treatError();
        }
        if (higherB <=  myBiopolymerClassContainer.updBiopolymerClass(parameterStringClass.getString(4)).sum (lowerB,1)) {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" for the second chain, specify first the higher numbered, then the lower numbered residue. Helix must be at least 3BP long."<<endl;
            ErrorManager::instance.treatError();
        }
        for (int i = 0 ; i <= myBiopolymerClassContainer.updBiopolymerClass(parameterStringClass.getString(1)).difference(higherA,lowerA); i ++) {
            ...
            {
                ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : No biopolymer found with chain ID "<<myBasePair.FirstBPChain<<".  Please declare all biopolymers before using them in this command.  "<<endl; 
                ErrorManager::instance.treatError();
            }
            if (! myBiopolymerClassContainer.hasChainID(myBasePair.SecondBPChain))
            {
                ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : No biopolymer found with chain ID "<<myBasePair.SecondBPChain<<".  Please declare all biopolymers before using them in this command.  "<<endl; 
                ErrorManager::instance.treatError();
            }
            basePairContainer.addBasePair(myBiopolymerClassContainer, _leontisWesthofClass, myBasePair, setHelicalStacking); 
        }
    ...
    if ( ((parameterStringClass.getString(0)).compare( "modifyBiopolymer") == 0)) {
        cout<<__FILE__<<":"<<__LINE__<<"  This command enables post transcriptional/translational modifications to canonical nucleic/amino acids.  You would first create a compound with the \"molecule\" command.  Then you would bond that compound to a specified biopolymer residue. "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<"  Syntax: modifyBiopolymer <biopolymer chain to modify> <residue to modify> <atom to modify> <free bond (e.g. bond1, bond2 ..)> <chain ID of compound to add> <atom on compound to be bonded> <free bond on added compound> "<<endl;
 ...
 //     cout << ((parameterStringClass.getString(0)) << endl;
    ...
    if ( ((parameterStringClass.getString(0)).compare("NtC") == 0)) {
     ...
     //   ErrorManager::instance<<"NtC-classes"<<endl;
     //   ErrorManager::instance<<"-----------"<<endl;
        
        NTC.NtC_FirstBPChain = (parameterStringClass.getString(1));
        NTC.NtC_step_ID = (parameterStringClass.getString(2));
        ...
        NTC.SecondBPResidue = myBiopolymerClassContainer.residueID(userVariables,parameterStringClass.getString(3).c_str(), NTC.NtC_FirstBPChain);
        
        NTC.weight          = stod(parameterStringClass.getString(5));
        NTC.meta            = 0;
        
        if((parameterStringClass.getString(6)).compare("meta") == 0){
            
            NTC.meta = 1;
            NTC.weight2     = stod(parameterStringClass.getString(7));
        ...
        if((parameterStringClass.getString(6)).compare("threebody") == 0){
            
            NTC.threebody = 1;
            
            NTC.sec_res_chain_threebody     = (parameterStringClass.getString(7));
            ...
            myBiopolymerClassContainer.count2++;               
            
        }  
  
        NTC.twobody = 0;
        
        if((parameterStringClass.getString(6)).compare("twobody") == 0){
            ...
            myBiopolymerClassContainer.count3++;            
    
            cout << NTC.count << " number of NTC twobody input lines " << endl;            
    ...
    { // if this is a base pair
        ...
        if (((parameterStringClass.getString(0)).compare("aromatic") == 0) ) {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : The \"aromatic\" command is no longer supported. "<<endl ; ErrorManager::instance.treatError();
        }
        if (((parameterStringClass.getString(0)).compare("baseInteraction") == 0) ) {
            if (parameterStringClass.getString(6).length() == 0) {
                ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You must specify the second interacting \"edge\" for the baseInteraction command. "<<endl ; ErrorManager::instance.treatError();
            }
            if (parameterStringClass.getString(7).length() == 0) {
                ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You must specify the orientation (typically Cis or Trans) for the baseInteraction command. "<<endl ; ErrorManager::instance.treatError();
            }
        }
        { 
            myBasePair.FirstBPChain = ((parameterStringClass.getString(1)));            
            ...
            if ((myBasePair.FirstBPEdge.compare("ChiBondAnti") ==0) ){cout<<__FILE__<<":"<<__LINE__<<" ChiBondAnti is not currently supported.  Making sure you don't use it.."<<endl; assert (0);}
            myBasePair.SecondBPChain = ((parameterStringClass.getString(4)));            
            ...
            if (parameterStringClass.getString(8).length() > 0) {  
                ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" Too many parameters!  Priorities are no longer supported for baseInteraction\'s. "<<endl;
                ErrorManager::instance.treatError();
            }
            ...
            if ((parameterStringClass.getString(0).compare("constraint") == 0) || (parameterStringClass.getString(0).compare("restraint") == 0)) 
                if (myBasePair.FirstBPEdge.compare("Weld")==0){ // all is well
                    ...
                    ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : The only type of "<<parameterStringClass.getString(0)<<" permitted is Weld.  You have specified : "<< myBasePair.FirstBPEdge<<endl;
                    ErrorManager::instance.treatError();
                }
            if (safeParameters) {
...

                {
                    ...
                    {ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You have not declared the chain "<<myBasePair.FirstBPChain<<" yet, or the chain is empty.  "<<endl; ErrorManager::instance.treatError();}
                    ...
                    {ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You have not declared the chain "<<myBasePair.SecondBPChain<<" yet, or the chain is empty.  "<<endl; ErrorManager::instance.treatError();}
                }
        ...
        r++; 
        if (verbose) cout<<__FILE__<<":"<<__LINE__<<" r incremented to "<<r<<endl;
        myBasePair.basePairSatisfied = "False"; //initialize
    ...
    { // if this is a mobilizer or constraint
        cout<<__FILE__<<":"<<__LINE__<<" Syntax: mobilizer <Bond Mobility> <chain> <start residue> <end residue>"<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" Or:     mobilizer <Bond Mobility> <chain> .. to set all residues in <chain> to <Bond Mobility>"<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" Or:     mobilizer <Bond Mobility>  .. to set all residues in all chains to <Bond Mobility>"<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" Where Bond Mobility may be RNA, DNA, or Protein"<<endl;
		...
		ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : The number of biopolymers detected at this point is  "<<myBiopolymerClassContainer.getNumBiopolymers() <<". Please call the mobilizer command after the chain(s) in question have been instantitated."<<endl ; 
		ErrorManager::instance.treatError();
            ...
            /*for (int i = 0 ; i < myBiopolymerClassContainer.getNumBiopolymers(); i++) {
                ...
                //cout<<__FILE__<<":"<<__LINE__<<" myMobilizerString = >"<<myMobilizerString<<"< "<<endl;
                cout<<__FILE__<<":"<<__LINE__<<" Adding mobilizer stretch to biopolymer index "<<i<<endl;
            ...
            cout<<__FILE__<<":"<<__LINE__<<" first residue ID: "<<    myBiopolymerClassContainer.residueID(userVariables,parameterStringClass.getString(3), parameterStringClass.getString(2)  ).outString()<<endl;
            cout<<__FILE__<<":"<<__LINE__<<" second residue ID: "<<    myBiopolymerClassContainer.residueID(userVariables,parameterStringClass.getString(4), parameterStringClass.getString(2)  ).outString()<<endl;
            ...
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You have specified the wrong number of parameters for this command. "<<endl ; 
            ErrorManager::instance.treatError();
        }
        return;
    } // End mobilizer
    ...
    if ( ((parameterStringClass.getString(0)).compare("initialDisplacement") == 0)  )    { 
        cout<<__FILE__<<":"<<__LINE__<<" Syntax: initialDisplacement <chain> <X> <Y> <Z> "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" Where <chain> is the chain to be displaced, and the next 3 parameters are the displacement vector."<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" You can also leave out <chain>, and all chains will be spread out in a line: "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" Syntax: initialDisplacement <X> <Y> <Z> "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" Where the next 3 parameters are the displacement vector, and each Biopolymer chain will be displaced by i *  (X,Y,Z) , where i is the chain index. This is particularly useful for spreading out the chains to make rendering easier."<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" Note that in MMB 2.10 and earlier, we took the displacement in Å.  We are going back to nm for consistency, with apologies for the confusion."<<endl;
    ...
    if ((parameterStringClass.getString(0)).compare("constrainChainRigidSegments") == 0) {
        cout<<__FILE__<<":"<<__LINE__<<" : To constrain all rigid segments in chain <chain ID> to a specified residue in that chain, issue:  "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" : Syntax:  constrainChainRigidSegments <chain ID> <residue ID to constrain to>  "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" : To constrain all rigid segments in chain <chain ID> to ground, issue:  "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" : Syntax:  constrainChainRigidSegments <chain ID> Ground   "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" : To constrain all rigid segments in all chains to ground, issue:  "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" : Syntax:  constrainChainRigidSegments   "<<endl;

        if (parameterStringClass.getString(3).length()>0){
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" You have provided too many parameters for this command."<<endl; ErrorManager::instance.treatError();
        } 
        cout<<__FILE__<<":"<<__LINE__<<endl;
        if (parameterStringClass.getString(2).length() > 0){
            //ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" You have not provided enough parameters for this command."<<endl; ErrorManager::instance.treatError();
            ...
            //ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : complete this later.."<<endl; ErrorManager::instance.treatError();
	    ...
	    //map<const String, BiopolymerClass>::iterator biopolymerClassMapIterator = myBiopolymerClassContainer.getBiopolymerClassMap().begin();
            cout<<__FILE__<<":"<<__LINE__<<" Detected that you wish to constrain rigid segments in the following :"<<endl;
            for (int i = 0; i < myBiopolymerClassContainer.getNumBiopolymers(); i++)
	    //for(biopolymerClassMapIterator = myBiopolymerClassContainer.getBiopolymerClassMap().begin(); biopolymerClassMapIterator != myBiopolymerClassContainer.getBiopolymerClassMap().end(); biopolymerClassMapIterator++) 
            {
                 cout<<__FILE__<<":"<<__LINE__<<" chain : "<<  myBiopolymerClassContainer.updBiopolymerClass(i).getChainID()  <<endl;
		 ChainResidueToGround myChainResidue;
		 ...
		 constraintToGroundContainer.queueConstrainChainRigidSegments(myChainResidue );
            ...
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" You have provided the wrong number of parameters for this command."<<endl; ErrorManager::instance.treatError();
        } else {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" Unexplained error! "<<endl; ErrorManager::instance.treatError();
        }
    ...
    if ((parameterStringClass.getString(0)).compare("constraint") == 0) {
        cout<<__FILE__<<":"<<__LINE__<<" : Syntax :  constraint <chain> <atom name> Weld Ground "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" :      This one works for  custom molecules only. "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" : Syntax :  constraint <chain 1> <residue ID 1> Weld Ground "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" :      This one works for  biopolymers only. "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" : Or:  constraint <chain 1> <residue ID 1> <atom name 1> Weld Ground "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" :      This one works for  biopolymers only. "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" : Or:  constraint <chain 1> <residue ID 1> Weld <chain 2> <residue ID 2> "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" : Or:  constraint <chain 1> <residue ID 1> <atom name 1> Weld <chain 2> <residue ID 2> <atom name 2> "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" :      This one treats atoms from monoAtoms and biopolymers on an equal footing. It does not work for custom molecules though. "<<endl;
        ...
        if ( (parameterStringClass.getString(4).length()>0) &&(!(parameterStringClass.getString(5).length() > 0))){  // This means syntax:  constraint <chain 1> <residue ID 1> Weld Ground
            ...
            if (myBiopolymerClassContainer.hasChainID(myChain1)) {
                cout<<__FILE__<<":"<<__LINE__<<" : detected syntax for biopolymers:  constraint <chain 1> <residue ID 1> Weld Ground "<<endl;
		...
		if (removeRigidBodyMomentum) {
		    ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You must first set removeRigidBodyMomentum False before welding to ground ! "<<endl;
		    ErrorManager::instance.treatError();
		}
		...
		myBiopolymerClassContainer.addConstraintToGround(userVariables, myResidue1String,myChain1,constraintToGroundContainer);
                ...
                cout<<__FILE__<<":"<<__LINE__<<" : Detected syntax for custom molecules:  constraint <chain> <atom name> Weld Ground "<<endl;
		if (!( (parameterStringClass.getString(3)).compare("Weld") == 0)) {
		    ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : Syntax error!"<<endl;
		    ErrorManager::instance.treatError(); 
		}
		...
		moleculeClassContainer.addConstraintToGround(userVariables,myChain1,myAtomName, constraintToGroundContainer);
		    ...
		    ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : Chain "<<myChain1<<" is neither a biopolymer nor a custom molecule!"<<endl;
		    ErrorManager::instance.treatError();
		}
		...
		ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : Wrong number of parameters! This syntax is not supported for now."<<endl;
		ErrorManager::instance.treatError();
	    }    
    ...
    if ((((parameterStringClass.getString(0)).compare(constrainInterfacesCommand) == 0))  ||
        ...
        {
        cout<<__FILE__<<":"<<__LINE__<<" Syntax variants and explanations follow :"<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" 1. Syntax : "<<parameterStringClass.getString(0) <<"  <depth (nm)>   "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" 2. Syntax :  "<<parameterStringClass.getString(0) <<"  <depth (nm)>  <chain>  "<<endl;
    
        cout<<__FILE__<<":"<<__LINE__<<" 3. Syntax :  "<<parameterStringClass.getString(0) <<"  <depth (nm)>  <chain 1> [<chain 2> [<chain 3> [ ...etc]]]  "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" 4. Syntax :  "<<parameterStringClass.getString(0) <<"  <depth (nm)>  <chain 1> [<chain 2> [<chain 3> [ ...etc]]]  Versus <chain 1> [<chain 2> [<chain 3> [ ...etc]]]  "<<endl;
        if (((parameterStringClass.getString(0)).compare(constrainInterfacesCommand) == 0)) {
	    cout<<__FILE__<<":"<<__LINE__<<" 1. This constrains all  chains to all  chains, if they are within a distance of <depth (nm)>. The constraint (always of type Weld) is applied to the pair of atoms spanning the two chains at closest approach (in terms of internuclear distance).  "<<endl;
	    cout<<__FILE__<<":"<<__LINE__<<" 2. This constrains the specified <chain> to all other chains within a distance of <depth (nm)>. The constraint (always of type Weld) is applied to the pair of atoms spanning the two chains at closest approach (in terms of internuclear distance).  "<<endl;
	    cout<<__FILE__<<":"<<__LINE__<<" 3. You can also specify a multiple-chain set, and constrain all specified chains to all the remaining chains in the system:"<<endl;
	    cout<<__FILE__<<":"<<__LINE__<<" 4. You can similarly specify TWO multiple-chain sets, and apply constraints between all possible pairs spanning the two sets. This is useful e.g. in the ribosome, where 23S binds a different set of proteins than 16S. "<<endl;}
        if (((parameterStringClass.getString(0)).compare(detectInterChainClashesCommand) == 0)) {
	    cout<<__FILE__<<":"<<__LINE__<<" 1. This detects clashes between all vs. all chains, if they are within a distance of <depth (nm)>.   "<<endl;
	    cout<<__FILE__<<":"<<__LINE__<<" 2. This detects clahes between the specified <chain> and all other chains within a distance of <depth (nm)>.  "<<endl;
	    cout<<__FILE__<<":"<<__LINE__<<" 3. You can also specify a multiple-chain set, and detect clashes between  all specified chains to all the remaining chains in the system:"<<endl;
	    cout<<__FILE__<<":"<<__LINE__<<" 4. You can similarly specify TWO multiple-chain sets, and detect clashes between all possible pairs spanning the two sets.  "<<endl;}
        ...
        if (parameterStringClass.getString(1).length() == 0) {
		ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" Not enough parameters specified for this command!"<<endl;
		ErrorManager::instance.treatError();
        }
        if (parameterStringClass.getString(2).length() == 0) {
            cout<<__FILE__<<":"<<__LINE__<<" No chains specified! This must mean you want to constrain all vs. all."<<endl; 
            // leave chains and partnerChains empty .. 
            /*for (int i = 0 ; i < myBiopolymerClassContainer.getNumBiopolymers(); i++) {
                chains.push_back       (myBiopolymerClassContainer.updBiopolymerClass(i).getChainID());
                cout<<__FILE__<<":"<<__LINE__<<" Added chain "<<myBiopolymerClassContainer.updBiopolymerClass(i).getChainID() <<" to first set "<<endl;
                partnerChains.push_back(myBiopolymerClassContainer.updBiopolymerClass(i).getChainID());
                cout<<__FILE__<<":"<<__LINE__<<" Added chain "<<myBiopolymerClassContainer.updBiopolymerClass(i).getChainID() <<" to second set "<<endl;
            }*/
        }
        ...
        while ((parameterStringClass.getString(i).length() >0)  &&
            ...
            cout<<__FILE__<<":"<<__LINE__<<" Added chain "<<parameterStringClass.getString(i)<<" to first set "<<endl;
            i++;
        }
        if (parameterStringClass.getString(i).toUpper().compare(versusString) == 0 ) { 
            i++;
            cout<<__FILE__<<":"<<__LINE__<<" Keyword "<<versusString<<" detected."<<endl;
            if (parameterStringClass.getString(i).length() == 0) {
		ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" The \"versus\" keyword must be followed by at least one chain ID."<<endl;
		ErrorManager::instance.treatError();
            }
        } 
        while (parameterStringClass.getString(i).length() >0) {
            ...
            cout<<__FILE__<<":"<<__LINE__<<" Added chain "<<parameterStringClass.getString(i)<<" to second set "<<endl;
            i++;
        }
        
        if (partnerChains.size() == 0) {
            cout<<__FILE__<<":"<<__LINE__<<" No partner chains specified! This must mean you want to constrain the first set vs. all possible chains."<<endl; 
        }
        if (((parameterStringClass.getString(0)).compare(constrainInterfacesCommand) == 0)) {
    ...
    if ((parameterStringClass.getString(0)).compare("coupleAtomMobilizers") == 0) {
        cout<<__FILE__<<":"<<__LINE__<<" : This constrains the Torsion mobilizers on two given atoms to be equal.   "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" : Syntax:  coupleAtomMobilizers <chain 1> <residue ID 1> <atom name 1> <chain 2> <residue ID 2> <atom name 2>  "<<endl;
    ...
    {   
        cout<<__FILE__<<":"<<__LINE__<<" : This constrains the Torsion mobilizers corresponding to Psi and Phi angles of <chain A>, residues  <start-residue-A> to <end-residue-A>, to be equal to the equivalent mobilizers in <Chain B>, residues <start-residue-B> to <end-residue-B>."<<endl;
        //cout<<__FILE__<<":"<<__LINE__<<" : This constrains the Torsion mobilizers of all atoms in <chain A>, residues  <start-residue-A> to <end-residue-A>, to be equal to the equivalent mobilizers in <Chain B>, residues <start-residue-B> to <end-residue-B>."<<endl;
        //cout<<__FILE__<<":"<<__LINE__<<" : Syntax:  coupleMobilizers <Chain A> <start-residue-A> <end-residue-A> <Chain-B> <start-residue-B> <end-residue-B> "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" : Syntax:  couplePsiPhiAngles <Chain A> <start-residue-A> <end-residue-A> <Chain-B> <start-residue-B> <end-residue-B> "<<endl;
        String      chainA = parameterStringClass.getString(1);
        String      chainB = parameterStringClass.getString(4);
        ResidueID startResidueA = myBiopolymerClassContainer.residueID(userVariables,parameterStringClass.getString(2),chainA );
        ResidueID   endResidueA = myBiopolymerClassContainer.residueID(userVariables,parameterStringClass.getString(3),chainA );
        ...
        else if (((parameterStringClass.getString(0)).compare("couplePsiPhiAngles")      == 0)) 
        {
            for (int i = 0; i < threadedLength; i++) 
            {
                ...
                for (int j = 0; j < 2; j++) 
                {
                    ...
                    {
                        ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : Something odd happened.   "<<endl;
                        ErrorManager::instance.treatError();
                    }
        ...
        {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : Something odd happened.  This is not a supported threading command. "<<endl;
            ErrorManager::instance.treatError();
        }
    ...
    if ((parameterStringClass.getString(0)).compare("rootMobilizer") == 0) {
        cout<<__FILE__<<":"<<__LINE__<<" : Syntax: rootMobilizer <\"Free\" | \"Weld\"> "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" : ... if you want to apply this root mobilizer to all biopolymer chains in the system "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" :     or: rootMobilizer <Chain> <\"Free\" | \"Weld\"> "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" : ... if you want to apply this root mobilizer to the specified Chain.                "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" : \"Free\" is the default, and confers 6 degrees of freedom to the root atom.  \"Weld\" grants the root atom 0 DOFs -- this behaves much like welding the root atom to ground but is computationally MUCH cheaper and numerically more accurate (no constraint tolerance to worry about). "<<endl;
        if ((parameterStringClass.getString(1).length() == 0) ) { ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : Not enough parameters for this command."<<endl; ErrorManager::instance.treatError();}
        if ((parameterStringClass.getString(2).length() == 0) ) {
            cout<<__FILE__<<":"<<__LINE__<<" : You have specified that all chains will be joined to ground with a "<<parameterStringClass.getString(1)<<" mobilizer"<<endl;                         
            ...
            cout<<__FILE__<<":"<<__LINE__<<" : You have specified that chain "<<parameterStringClass.getString(1)<<" will be joined to ground with a Weld mobilizer"<<endl;
        ...
        } else { ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : Too many parameters for this command!"<<endl; ErrorManager::instance.treatError();}
        return;

    }

    if ((parameterStringClass.getString(0)).compare("constrainToGround") == 0) {
        cout<<__FILE__<<":"<<__LINE__<<" : Syntax: constrainToGround <chain> <ResidueID> "<<endl;
        //cout<<__FILE__<<":"<<__LINE__<<" : Or: constrainToGround  "<<endl;
        //cout<<__FILE__<<":"<<__LINE__<<" : In the latter case, all first residues will be have no degrees of freedom with respect to ground.  This is very efficient! It is not the same as constraining all first residues to ground.  "<<endl;

        if (removeRigidBodyMomentum) {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You must first set removeRigidBodyMomentum False before using this command! "<<endl;
            ErrorManager::instance.treatError();
        }
        if ((parameterStringClass.getString(1).length() == 0) ) {
            //firstResidueMobilizerType = "Weld";
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" You have tried to use an obsolete syntax.  Please use the rootMobilizer command."<<endl; ErrorManager::instance.treatError();
            cout<<__FILE__<<":"<<__LINE__<<" : You have specified that all chains will be joined to ground with a Weld mobilizer"<<endl;
            myBiopolymerClassContainer.setFirstResidueMobilizerType(String("Weld"));

        } else if ((parameterStringClass.getString(2).length() == 0) ) {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" You have tried to use an obsolete syntax.  Please use the rootMobilizer command."<<endl; ErrorManager::instance.treatError();
            cout<<__FILE__<<":"<<__LINE__<<" : You have specified that chain "<<parameterStringClass.getString(2)<<" will be joined to ground with a Weld mobilizer"<<endl;
            ...
            if ( myBiopolymerClassContainer.updBiopolymerClass(parameterStringClass.getString(1)).getFirstResidueID() == 
                    myBiopolymerClassContainer.residueID(userVariables, parameterStringClass.getString(2), parameterStringClass.getString(1)) )  
                        cout<<__FILE__<<":"<<__LINE__<<" : Looks like you're welding the first residue to ground.  Please consider using the rootMobilizer command -- it's more accurate and much cheaper."<<endl;
    ...
    {
        cout<<__FILE__<<":"<<__LINE__<<" : Syntax for biopolymers: addRingClosingBond  <chainID> <residueID1> <atomName1> <bondCenterName1>  <residueID2>  <atomName2> <bondCenterName2> "<<std::endl;
        cout<<__FILE__<<":"<<__LINE__<<" : Syntax for molecule objects: addRingClosingBond  <chainID> <atomName1> <bondCenterName1> <atomName2> <bondCenterName2> "<<std::endl;
	...
	CovalentBondClass myBond;
		...
		additionalCovalentBondVector.push_back(myBond);
                ...
                ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : Error! chain >"<<chainID<<"< is not a biopolymer or molecule! Or possibly not enough parameters specified."<<endl ; ErrorManager::instance.treatError();
        }
    ...
    { // if this is a restraint or constraint
        ...
        if (((parameterStringClass.getString(0)).compare("restrainToGround") == 0)  ) 
        {
            if (removeRigidBodyMomentum)
            {
                ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : Please set \"removeRigidBodyMomentum FALSE\" before issuing the constrainToGround or restrainToGround commands!"<<endl ; ErrorManager::instance.treatError();
            }
        ...
        { 
            ...
            myBasePair.OrientationBP = parameterStringClass.getString(7);           
            if (parameterStringClass.getString(8).length() > 0) {  
                ErrorManager::instance <<__FILE__<<":"<<__LINE__<<"Too many parameters!  Check your syntax.  This command no longer accepts a Stage parameter. "<<endl;
                ErrorManager::instance.treatError();
            }
            else { 
                cout<<__FILE__<<":"<<__LINE__<<"Stage of command automatically set to stage 1."<<endl;
                myBasePair.BasePairPriority = 1; 
            }
    ...
    if  (((parameterStringClass.getString(0)).compare("contact") == 0))  {
        cout<<__FILE__<<":"<<__LINE__<<"  Current syntax of the \"contact\" command is:  contact <contact scheme> <chain> <start residue> <end residue> "<<endl; 
        cout<<__FILE__<<":"<<__LINE__<<"  Or:  contact <contact scheme> <chain> <residue>  "<<endl; 
        cout<<__FILE__<<":"<<__LINE__<<"  Or:  contact <contact scheme> <chain>  "<<endl; 
        ...
        if (parameterStringClass.getString(5).length() >0) {
            ...
            //ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You have specified the wrong number of parameters for this command. "<<endl; ErrorManager::instance.treatError();
        } else if ((parameterStringClass.getString(2).length() >0 ))   
        {
            ContactStretch myContactStretch;
            myContactStretch.ContactScheme = parameterStringClass.getString(1);
    ...
    if  (((parameterStringClass.getString(0)).compare("fitToDensity") == 0))  {
        ...
        cout<<__FILE__<<":"<<__LINE__<<"  Current syntax of  command is:  fitToDensity  <chain> "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<"  Or :  fitToDensity  <chain> <first residue> <last residue> "<<endl;

        if ((dutyCycle<1.0) ) {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" you can't set dutyCycle<1.0 if you also issue the fitToDensity command"<<endl; ErrorManager::instance.treatError();
        }
        ...
        if (parameterStringClass.getString(1).length() == 0) {
            cout<<__FILE__<<":"<<__LINE__<<" : You have requested that all biopolymer chains be fitted to the density map. "<<endl; 
            if (myBiopolymerClassContainer.getNumBiopolymers() == 0) {
                ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : ERROR: This command can only be issued AFTER you have declared or instantiated your biopolymer chains."<<endl; ErrorManager::instance.treatError();
            }

            densityContainer.stuffDensityStretchVector(myBiopolymerClassContainer);
        }
        else if (parameterStringClass.getString(2).length() == 0) {
            if (parameterStringClass.getString(1).length() == 0) {ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You have not specified enough parameters for this command. "<<endl; ErrorManager::instance.treatError();}
            ...
            if (parameterStringClass.getString(3).length() == 0) {ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You have not specified enough parameters for this command. "<<endl; ErrorManager::instance.treatError();}
        ...
        } else {ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You have specified too many parameters for this command."<<endl; ErrorManager::instance.treatError(); } // field 5 is not empty
        cout<<__FILE__<<":"<<__LINE__<<" : During reading of input file, printing updated list of density stretches: "<<endl;
    ...
    if  (((parameterStringClass.getString(0)).compare("fitElectrostaticDensity") == 0))  {
        ...
        cout<<__FILE__<<":"<<__LINE__<<"  Current syntax of  command is:  fitElectrostaticDensity  <chain> "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<"  Or :  fitElectrostaticDensity  <chain> <first residue> <last residue> "<<endl;

        if ((dutyCycle<1.0) ) {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" you can't set dutyCycle<1.0 if you also issue the fitElectrostaticDensity command"<<endl; ErrorManager::instance.treatError();
        }
    ...
    if  (((parameterStringClass.getString(0)).compare("applyContactsWithin") == 0))  {
        #ifndef USE_OPENMM
        ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You have compiled without USE_OPENMM ."<<endl; ErrorManager::instance.treatError(); 
        #endif
        cout<<__FILE__<<":"<<__LINE__<<"  Current syntax of  command is:  applyContactsWithin <radius (nm)> <contact scheme> <chain> <residue>  "<<endl; 
        cout<<__FILE__<<":"<<__LINE__<<" Note that in MMB 2.10 and earlier, we took the radius in Å.  We are going back to nm for consistency, with apologies for the confusion."<<endl;
    ...
    if  (((parameterStringClass.getString(0)).compare("applyMobilizersWithin") == 0))  {
        cout<<__FILE__<<":"<<__LINE__<<" Current syntax of  command is:  applyMobilizersWithin <Bond Mobility> <radius (nm)>  <chain> <residue>  "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" Note that in MMB 2.10 and earlier, we took the radius in Å.  We are going back to nm for consistency, with apologies for the confusion."<<endl;
    ...
    if  (((parameterStringClass.getString(0)).compare("mobilizeInterfaces") == 0))  {
        cout<<__FILE__<<":"<<__LINE__<<" Syntax is:  mobilizeInterfaces <depth (nm)> <bond mobility> <chain>  "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" This sets bond mobility to <bond mobility>  for all residues within <depth> of <chain> in all other chains.  It also sets the bond mobility for residues in <chain> that are within <depth> of that other chain -- so the interface is treated symmetrically."<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" You can also specify a multiple-chain set, and find all interfaces between that set and the remainder of chains in the system:"<<endl;
    
        cout<<__FILE__<<":"<<__LINE__<<" Syntax is:  mobilizeInterfaces <depth (nm)> <bond mobility> <chain 1> [<chain 2> [<chain 3> [ ...etc]]]  "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" Lastly, you can specify TWO multiple-chain sets, and find only the all interface between those two sets.  This is particularly useful if you have chains (e.g. threading templates) in your system which need to be ignored:"<<endl;
    
        cout<<__FILE__<<":"<<__LINE__<<" Syntax is:  mobilizeInterfaces <depth (nm)> <bond mobility> <chain 1> [<chain 2> [<chain 3> [ ...etc]]]  Versus <chain 1> [<chain 2> [<chain 3> [ ...etc]]]  "<<endl;

        if (parameterStringClass.getString(3).length() == 0) {
            cout<<__FILE__<<":"<<__LINE__<<" Not enough parameters for this command! You must specify at least one chain."<<endl; exit(1);
        }
        double depth = myAtoF(userVariables,parameterStringClass.getString(1).c_str());
	    ...
	    ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" The reference chain "<<chains[n]<<" is the same as partner chain "<<partnerChains[m]<<". This is not kosher!"<<endl;
	    ErrorManager::instance.treatError();
        }
    ...
    {
        cout<<__FILE__<<":"<<__LINE__<<" Syntax is:  mobilizeDomainsInterface <depth (nm)> <bond mobility> <chain domain 1> <resStart1> <resEnd1> <chain domain 2> <resStart2> <resEnd2> [<'backbone-rigid'>] "<<endl;
        if (parameterStringClass.getString(8).length() == 0) {
            ErrorManager::instance<<__FILE__<<":"<<__LINE__<<" Not enough parameters for this command!"<<endl;
            ErrorManager::instance.treatError();
        }
    ...
    if  (((parameterStringClass.getString(0)).compare("readInQVector") == 0))  {
        parameterStringClass.validateNumFields(2);
        ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" The readInQVector is obsolete."<<endl;
        ErrorManager::instance.treatError();
        readInQVector = myAtoI(userVariables,(parameterStringClass.getString(1)).c_str());     
        readPreviousFrameFile = (!(readInQVector));
        if (verbose) cout<<__FILE__<<":"<<__LINE__<<"  readInQVector ="<<readInQVector<<endl; 
        return;
    }
    ...
    if  (((parameterStringClass.getString(0)).compare("-FS") == 0) || ((parameterStringClass.getString(0)).compare("firstStage") == 0))  {
        ...
        if (readStage || readAtOneStageOnly ||readOnlyUntilStage || readExcept ) {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" You are not allowed to set the firstStage or lastStage parameter in a conditional block."<<endl;
            ErrorManager::instance.treatError();
        }
        if (verbose) cout<<__FILE__<<":"<<__LINE__<<" firstStage ="<<firstStage<<endl; 
        return;
    }
    if  (((parameterStringClass.getString(0)).compare( "periodicallyUpdateParameters") == 0))  {
        parameterStringClass.validateNumFields(2);
        ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" The periodicallyUpdateParameters is obsolete."<<endl;
        ErrorManager::instance.treatError();
        periodicallyUpdateParameters = myAtoI(userVariables,(parameterStringClass.getString(1)).c_str());     
        if (verbose) cout<<__FILE__<<":"<<__LINE__<<"  periodicallyUpdateParameters ="<<periodicallyUpdateParameters<<endl; 
        return;
    }
    if  (((parameterStringClass.getString(0)).compare( "lastStage") == 0))  {
        ...
        if (verbose) cout<<__FILE__<<":"<<__LINE__<<"  lastStage ="<< lastStage<<endl; 
        return;
    }
    if  (((parameterStringClass.getString(0)).compare( "physicsRadius"  ) == 0))  {
        cout<<__FILE__<<":"<<__LINE__<<" If >0, MMB will include all residues within the provided radius of 'flexible' atoms, in the physics zone. 'flexible' is defined as atoms belonging to low-mass (<40 au) groups."<<endl; 
        cout<<__FILE__<<":"<<__LINE__<<" Distance is determined using OpenMM's neighborlisting function, so is on basis of nearest atoms, and is quite efficient"<<endl; 
        cout<<__FILE__<<":"<<__LINE__<<" Syntax:  physicsRadius <radius>"<<endl;
        parameterStringClass.validateNumFields(2);
        physicsRadius = myAtoF(userVariables,(parameterStringClass.getString(1)));     
        if (physicsRadius < 0.0) {cout<<__FILE__<<":"<<__LINE__<<" physicsRadius must be >0 !"<<endl; exit(1);}
        if (verbose) cout<<__FILE__<<":"<<__LINE__<<"  physicsRadius ="<< physicsRadius<<endl; 
        return;
    }
    ...
    if(parameterStringClass.getString(0).compare("deactivatePhysics") == 0) 
    {
        ...
        cout << __FILE__<<":"<<__LINE__<<" Chain " << chain << " will not be included in any physics or steric interactions." << endl;
    ...
    if(parameterStringClass.getString(0).compare("activatePhysics") == 0) 
    {
        ...
        String chain = parameterStringClass.getString(1);
        BiopolymerClass & bpc = myBiopolymerClassContainer.updBiopolymerClass(chain);
        cout << __FILE__<<":"<<__LINE__<<" Chain " << chain << " will now be included in requested physics or steric interactions." << endl;
    ...
    if  (((parameterStringClass.getString(0)).compare( "piecewiseRigidify") == 0))  {
        ...
        if (verbose) cout<<__FILE__<<":"<<__LINE__<<"  piecewiseRigidify ="<< piecewiseRigidify<<endl; 
        return;
    }

    if  (((parameterStringClass.getString(0)).compare( "planarityThreshold") == 0))  {
        cout<<__FILE__<<":"<<__LINE__<<" This parameter sets the threshold for considering the chirality to be of type Planar.  Units: radians."<<endl;
        parameterStringClass.validateNumFields(2);
        planarityThreshold = myAtoF(userVariables,(parameterStringClass.getString(1)));     
        if (planarityThreshold <= 0) {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" This angle should be positive."<<endl; ErrorManager::instance.treatError();
        }

        if (planarityThreshold >45 ) {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" Doesn't make sense for this angle to be so large."<<endl; ErrorManager::instance.treatError();
        }
        if (verbose) cout<<__FILE__<<":"<<__LINE__<<"  planarityThreshold="<<planarityThreshold<<endl; 
        return;
    }
    if  (((parameterStringClass.getString(0)).compare( "potentialType"    ) == 0))  {
        parameterStringClass.validateNumFields(2);
        potentialType     = ((parameterStringClass.getString(1)));     
        if (verbose) cout<<__FILE__<<":"<<__LINE__<<"  potentialType     ="<< potentialType    <<endl; 
        return;
    }
    ...
    if  (((parameterStringClass.getString(0)).compare( "magnesiumIonRadius") == 0))  {
        ...
        if (verbose) cout<<__FILE__<<":"<<__LINE__<<"  magnesiumIonRadius ="<< magnesiumIonRadius<<endl; 
        return;
    }
    if  (((parameterStringClass.getString(0)).compare( "matchDefaultSkipTopLevelTransform") == 0))  {
        ...
        if (verbose) cout<<__FILE__<<":"<<__LINE__<<"  matchDefaultSkipTopLevelTransform ="<< matchDefaultSkipTopLevelTransform<<endl; 
        return;
    }
    if  (((parameterStringClass.getString(0)).compare( "numMagnesiumIons") == 0))  {
        parameterStringClass.validateNumFields(2);
        ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" The numMagnesiumIons parameter is obsolete.  Please use the new monoAtoms command."<<endl;
        ErrorManager::instance.treatError();
        numMagnesiumIons = myAtoI(userVariables,(parameterStringClass.getString(1)).c_str());     
        if (verbose) cout<<__FILE__<<":"<<__LINE__<<"  numMagnesiumIons ="<< numMagnesiumIons<<endl; 
        return;
    }
    if  (((parameterStringClass.getString(0)).compare( "safeParameters") == 0))  {
        ...
        if (verbose) cout<<__FILE__<<":"<<__LINE__<<"  safeParameters ="<<safeParameters <<endl; 
        return;
    }
    ...
    if  (((parameterStringClass.getString(0)).compare( "setOverallBondMobility") == 0))  {
        ...
        if (verbose) cout<<__FILE__<<":"<<__LINE__<<"  setOverallBondMobility ="<< setOverallBondMobility<<endl; 
        return;
    }
    ...
    if  (((parameterStringClass.getString(0)).compare( "kbBackboneTorsionGlobalScaleFactor") == 0))  {
        parameterStringClass.validateNumFields(2);
        ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" The kbBackboneTorsionGlobalScaleFactor obsolete."<<endl;
        ErrorManager::instance.treatError();
    ...
    if  (((parameterStringClass.getString(0)).compare( "twoTransformForceMultiplier") == 0) ||((parameterStringClass.getString(0)).compare( "forceMultiplier") == 0) || ((parameterStringClass.getString(0)).compare( "baseInteractionScaleFactor") == 0) || ((parameterStringClass.getString(0)).compare( "baseInteractionForceMultiplier") == 0) )  {
        ...
        //     ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" The forceMultiplier (aliases: twoTransformForceMultiplier, baseInteractionScaleFactor, baseInteractionForceMultiplier) must be set before specifying any restraint, constraint, mobilizer, contact, constrainToGround, or restrainToGround command."<<endl;
        //     ErrorManager::instance.treatError();
        // }

        // if (  (basePairContainer.numBasePairs() >0)) {
        //     ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" The forceMultiplier (aliases: twoTransformForceMultiplier, baseInteractionScaleFactor, baseInteractionForceMultiplier) must be set before issuing any baseInteraction command."<<endl;
        //     ErrorManager::instance.treatError();
        // }     
        return;
    }
    if  (((parameterStringClass.getString(0)).compare( "tinkerParameterFileName") == 0))  {
        parameterStringClass.validateNumFields(2);
        if (myBiopolymerClassContainer.getNumBiopolymers() > 0){
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : The tinkerParameterFileName parameter can only be set prior to instantiating the first biopolymer. "<<endl;
            ErrorManager::instance.treatError();
        } 
        tinkerParameterFileName =((parameterStringClass.getString(1)));     
        if (verbose) cout<<__FILE__<<":"<<__LINE__<<" tinkerParameterFileName ="<<tinkerParameterFileName<<endl; 
        loadTinkerParameterFile = 1   ;
        cout<<__FILE__<<":"<<__LINE__<<" Since you specified tinkerParameterFileName, we are setting loadTinkerParameterFile = "<<loadTinkerParameterFile<<endl;
    ...
    if  (((parameterStringClass.getString(0)).compare( "integratorType"         ) == 0))  {
        ...
        if (verbose) cout<<__FILE__<<":"<<__LINE__<<" integratorType          ="<<integratorType         <<endl; 
    ...
    if  (((parameterStringClass.getString(0)).compare("readPreviousFrameFile") == 0))  {
        ...
        if (verbose) cout<<__FILE__<<":"<<__LINE__<<"  readPreviousFrameFile ="<< readPreviousFrameFile<<endl; 
        return;
    }
    if  (((parameterStringClass.getString(0)).compare("-T")  == 0) || ((parameterStringClass.getString(0)).compare("temperature")  == 0))   {
        cout<<__FILE__<<":"<<__LINE__<<": syntax: temperature <temperature>"<<endl;
        parameterStringClass.validateNumFields(2);
        if (parameterStringClass.getString(2).length() >  0) {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You have provided too many parameters for this command. temperature is no longer a staged parameter."<<endl; ErrorManager::instance.treatError();
        }
    ...
    if  (((parameterStringClass.getString(0)).compare("-DC") == 0) || ((parameterStringClass.getString(0)).compare("dutyCycle") == 0))  {
        ...
        if (dutyCycle < 1.00) {
            ...
            cout<<__FILE__<<":"<<__LINE__<<" You have set dutyCycle to "<<dutyCycle<<" .  Therefore, setForceAndStericScrubber has been set to "<<setForceAndStericScrubber <<endl; 
        }
        if (dutyCycle == 1.00) {
        ...
        {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" Error! dutyCycle must lie in the open interval (0,1) ! You have tried to set it to : "<<dutyCycle <<endl;  
            ErrorManager::instance.treatError();
        }
        ...
        //if (verbose) cout<<__FILE__<<":"<<__LINE__<<" dutyCycleArray["<<d <<"] ="<<dutyCycleArray[d]<< "from String :"<<parameterStringClass.getString(1)<<endl;  
        //if (verbose) cout<<__FILE__<<":"<<__LINE__<<" dutyCycleArray["<<d <<"] ="<<dutyCycleArray[d]<< "from String :"<<parameterStringClass.getString(1)<<endl;  
        //d++;
        return;
    }
    ...
    if (((parameterStringClass.getString(0)).compare("proteinCapping") ==0))  {
        parameterStringClass.validateNumFields(2);
        if (myBiopolymerClassContainer.getNumBiopolymers() >0) {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You must specify the proteinCapping parameter BEFORE you specify the first biopolymer (DNA, RNA, protein, etc.)"<<endl;
            ErrorManager::instance.treatError();
        }
    ...
    if (((parameterStringClass.getString(0)).compare("cutoffAngle") ==0)  )  {
        ...
        if (verbose) cout<< __FILE__<<":"<<__LINE__<<"  cutoffAngle ="<<cutoffAngle<<endl;
        return;
    }
    if (((parameterStringClass.getString(0)).compare("cutoffRadius") ==0)  )  {
        ...
        if (verbose) cout<< __FILE__<<":"<<__LINE__<<"  cutoffRadius ="<<cutoffRadius<<" nm "<<endl;
        return;
    }
    if (((parameterStringClass.getString(0)).compare("densityAtomFraction") ==0)  )  {
        ...
        if ((densityAtomFraction < 0.0) || (densityAtomFraction > 1.0)) {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" densityAtomFraction must be in the range of 0 to 1."<<endl;
            ErrorManager::instance.treatError();
        }
        return;
    }
    ...
    if (((parameterStringClass.getString(0)).compare("densityMapActivate") ==0)  )  {
        ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : This parameter is no longer supported.  You should now just use the fitToDensity command.. see the reference guide."<<endl;
        ErrorManager::instance.treatError();
        /*parameterStringClass.validateNumFields(2);
          if ((densityContainer.numDensityStretches()>0)) {
          ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You must specify all fitToDensity commands AFTER setting the densityMapActivate parameter."<<endl;
          ErrorManager::instance.treatError();
          }
          if ((removeRigidBodyMomentum)) {
          ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : If you want to activate the density based force field, you must first set \"removeRigidBodyMomentum FALSE\".  The order in which these parameters appear in the input file matters."<<endl; ErrorManager::instance.treatError();
          }
          ...
          ErrorManager::instance <<__FILE__<<":"<<__LINE__<<": You must set setForceAndStericScrubber FALSE before setting densityMapActivate TRUE.  "<<parameterFileName<<endl;
          ErrorManager::instance.treatError();
          }*/
        return;
    }
    ...
    if (((parameterStringClass.getString(0)).compare("vanderWallSphereRadius") ==0)  )  {
        ...
        if (verbose) cout<< __FILE__<<":"<<__LINE__<<" vanderWallSphereRadius ="<<vanderWallSphereRadius<<endl;
    ...
    if (((parameterStringClass.getString(0)).substr(0,1)).compare("@") ==0)  { 
        ...
        if (((parameterStringClass.getString(0)).compare("@CURRENTSTAGE") == 0)) {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : @CURRENTSTAGE is a reserved variable, you cannot change it!"<<endl;
            ErrorManager::instance.treatError();

        }
        cout<<__FILE__<<":"<<__LINE__<<" : read variable "<<parameterStringClass.getString(0) <<", successfully set to "<<userVariables[parameterStringClass.getString(0)]<<endl;
    ...
    if ((parameterStringClass.getString(0)).compare("fitDefaultTolerance"         ) ==0) {
        ...
        ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : read variable "<<parameterStringClass.getString(0) <<".  This is an obsolete parameter!"<<endl; ErrorManager::instance.treatError();
        parameterStringClass.validateNumFields(2);
    ...
    if ((parameterStringClass.getString(0)).compare("globalGbsaScaleFactor") ==0) {
        ...
        if ((!(fabs(globalGbsaScaleFactor) <= 0.0000001)) && safeParameters ) {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : Gbsa isn't supported in this release.  Please set globalGbsaScaleFactor 0.  You can set safeParameters FALSE to override this message. "<<endl; ErrorManager::instance.treatError();
        }   
        return;
    }
    ...
    if ((parameterStringClass.getString(0)).compare("waterDroplet") ==0) {
        cout<<__FILE__<<":"<<__LINE__<<"syntax:  waterDroplet <droplet chainID>  X Y Z radius [tetherStrength]"<<endl;
        //cout<<__FILE__<<":"<<__LINE__<<"Alternatively, one can specify an atom as the center. The tethers will not be applied to the atom, but rather to the Ground at the atom's initial position."<<endl;
        //cout<<__FILE__<<":"<<__LINE__<<"syntax:  waterDroplet <droplet chainID> AtomLocationInGround <atom chain ID> <atom residue number> <atom name> radius [tetherStrength [tetherType]]"<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" Note that in MMB 2.10 and earlier, we took the radius in Å.  We are going back to nm for consistency, with apologies for the confusion."<<endl;
	...
	myWaterDroplet.center[2] = myAtoF(userVariables,(parameterStringClass.getString(4)).c_str());
        ...
        cout<<__FILE__<<":"<<__LINE__<<" Setting water droplet radius to : "<<myWaterDroplet.getRadius()<<endl;
        if (myWaterDroplet.getRadius() <=0) {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" Droplet radius = "<<myWaterDroplet.getRadius()<<" .. less than zero!"<<endl; ErrorManager::instance.treatError();
        }
        if (parameterStringClass.getString(6).length() > 0) myWaterDroplet.tetherStrength = myAtoF(userVariables,(parameterStringClass.getString(6)).c_str());        cout<<__FILE__<<":"<<__LINE__<<" Setting tether strength to "<<myWaterDroplet.tetherStrength<<endl;

        if (!(parameterStringClass.getString(5).length() > 0)) {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : Invalid syntax!  You don't have enough parameters."<<endl;
            ErrorManager::instance.treatError(); 
        }

        if (myBiopolymerClassContainer.hasChainID(myWaterDroplet.chainID)) {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : Can't use chain "<<myWaterDroplet.chainID<<" to identify this water droplet, because it is already being used by a biopolymer in your system."<<endl;
            ErrorManager::instance.treatError(); 
        }
        for (int i = 0 ; i < (int)waterDropletAboutResidueVector.size(); i++) {
            if (waterDropletAboutResidueVector[i].waterDropletChainID.compare(myWaterDroplet.chainID) == 0) {    
                ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : Can't use chain "<<myWaterDroplet.chainID<<" to identify this water droplet, because it is already being used by another water droplet."<<endl;
                ErrorManager::instance.treatError();
            }
        } 
    ...
    if ((parameterStringClass.getString(0)).compare("waterDropletAboutResidue") ==0) {
        ...
        if (! (parameterStringClass.getString(5)).length()) {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : Please specify a valid chain ID to designate this water droplet."<<endl;
            ErrorManager::instance.treatError(); 
        }
        ...
        if ( (parameterStringClass.getString(6)).length()) {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You have specified too many parameters for this water droplet."<<endl;
            ErrorManager::instance.treatError(); 
        }
        if (myBiopolymerClassContainer.hasChainID(myWaterDropletAboutResidueStruct.waterDropletChainID)) {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : Can't use chain "<<myWaterDropletAboutResidueStruct.waterDropletChainID<<" to identify this water droplet, because it is already being used by a biopolymer in your system."<<endl;
            ErrorManager::instance.treatError(); 
        }
        for (int i = 0 ; i < (int)waterDropletAboutResidueVector.size(); i++) {
            if (waterDropletAboutResidueVector[i].waterDropletChainID.compare(myWaterDropletAboutResidueStruct.waterDropletChainID) == 0) {    
                ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : Can't use chain "<<myWaterDropletAboutResidueStruct.waterDropletChainID<<" to identify this water droplet, because it is already being used by another water droplet."<<endl;
                ErrorManager::instance.treatError();
            } 
        } 
    ...
    if ((parameterStringClass.getString(0)).compare("wkdpGlobalBondTorsionScaleFactor") ==0) {
        parameterStringClass.validateNumFields(2);
        ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" The wkdpGlobalBondTorsionScaleFactor is obsolete."<<endl;
        ErrorManager::instance.treatError();
    ...
    if ((parameterStringClass.getString(0)).compare("writeFrameFile"  ) ==0) {
        parameterStringClass.validateNumFields(2);
        ErrorManager::instance <<__FILE__<<":"<<__LINE__<<"writeFrameFile is obsolete."<<endl;
        ErrorManager::instance.treatError();
    ...
    if ((parameterStringClass.getString(0)).compare("rigidifyFormedHelices") ==0) {
        ...
        ErrorManager::instance <<__FILE__<<":"<<__LINE__<<": This is no longer a user configurable parameter.  .  "<<endl; ErrorManager::instance.treatError();
        return;
    }
    ...
    if 
        (
         ...
         ((parameterStringClass.getString(0)).compare("addRNABackboneSterics")     ==0)  
            ...
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : This command is no longer supported!  Instead, use the contact command, for each chain and stretch of residues you want to apply it to."<<endl; 
            ErrorManager::instance.treatError(); 
        }
    if 
        (
         ...
         //((parameterStringClass.getString(0)).compare("addRNABackboneSterics")     ==0)  
            ...
            {ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You may only set the addAllHeavyAtomSterics parameter to TRUE once, and afterwards cannot set it to FALSE."; ErrorManager::instance.treatError(); }
            if (addAllAtomSterics &&  ((parameterStringClass.getString(0)).compare("addAllAtomSterics") ==0))                
            {ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You may only set the addAllAtomSterics         parameter to TRUE once, and afterwards cannot set it to FALSE."; ErrorManager::instance.treatError(); }
            if (addSelectedAtoms  &&  ((parameterStringClass.getString(0)).compare("addSelectedAtoms") ==0))             
            {ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You may only set the addSelectedAtoms          parameter to TRUE once, and afterwards cannot set it to FALSE."; ErrorManager::instance.treatError(); }
            if (addProteinBackboneSterics &&   ((parameterStringClass.getString(0)).compare("addProteinBackboneSterics") ==0)) 
            {ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You may only set the addProteinBackboneSterics parameter to TRUE once, and afterwards cannot set it to FALSE."; ErrorManager::instance.treatError(); }
            if (addRNABackboneSterics &&   ((parameterStringClass.getString(0)).compare("addRNABackboneSterics") ==0)) 
            {ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You may only set the addRNABackboneSterics parameter to TRUE once, and afterwards cannot set it to FALSE."; ErrorManager::instance.treatError(); }
    ...
    if ((parameterStringClass.getString(0)).compare("addNASTForces") ==0) {
        parameterStringClass.validateNumFields(2);
        ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : The addNASTForces command has been discontinued."<<endl;
        ErrorManager::instance.treatError();
    ...
    if ((parameterStringClass.getString(0)).compare("addBackboneOxygenForces") ==0) {
        ...
        if (safeParameters) {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : addBackboneOxygenForces is no longer a supported parameter."<<endl;
            ErrorManager::instance.treatError();    
        }
        return;
    }
    if ((parameterStringClass.getString(0)).compare("addExcludedVolume"      ) ==0) {
        parameterStringClass.validateNumFields(2);
        ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : The addExcludedVolume command is no longer supported."<<endl;
        ErrorManager::instance.treatError();
        return;
    }
    if ((parameterStringClass.getString(0)).compare("loadTinkerParameterFile") ==0) {
	ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : The loadTinkerParameterFile parameter is false by default, but is set to TRUE upon setting tinkerParameterFileName . You are no longer permitted to set loadTinkerParameterFile directly!. "<<endl;
	ErrorManager::instance.treatError();
        
        /*parameterStringClass.validateNumFields(2);
        if (myBiopolymerClassContainer.getNumBiopolymers() > 0){
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : The loadTinkerParameterFile parameter can only be set prior to instantiating the first biopolymer. "<<endl;
            ErrorManager::instance.treatError();
        } 
    ...
    if (((parameterStringClass.getString(0)).compare("loadSequencesFromPdb") ==0) ||
        ...
        cout<<__FILE__<<":"<<__LINE__<<" Syntax is:  loadSequencesFromPdb [input structure file name] "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" If the optional structure file is omitted, sequences will be read from last.(n-1).pdb (n being the current stage). "<<endl;
        
        if (currentStage == 1) {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You can't call loadSequencesFromPdb at stage 1! This stage does not use any input structure file."<<endl; 
            ErrorManager::instance.treatError();
        }
        if(parameterStringClass.getString(3).length() > 0) 
        {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : loadSequencesFromPdb takes only two optional parameter which is a pdb file name and a chains prefix."<<endl;
            ErrorManager::instance.treatError();
        }
        if ((currentStage > firstStage) && (myBiopolymerClassContainer.getNumBiopolymers() >0) )
        {
            //cout <<__FILE__<<":"<<__LINE__<<" :We are not on first stage and some polymers are already been instantied, "<<previousFrameFileName<<" from previous stage has already been read. No need to do it again." << endl; 
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : We are not on first stage and some polymers are already been instantied, "<<previousFrameFileName<<" from previous stage has already been read. No need to do it again." << endl;// at stage 1! This stage does not use any input structure file."<<endl; 
            ErrorManager::instance.treatError();
            //return;
        }
        ...
        else {ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : Unexplained error!"<<endl; exit(1); }
        ...
        {
            cout <<__FILE__<<":"<<__LINE__<<" : We load " << previousFrameFileName << " from previous stage." << endl;
            pdbFileName = previousFrameFileName; 
            //cout <<__FILE__<<":"<<__LINE__<<" : We are not on first stage so we load " << previousFrameFileName << " from previous stage." << endl;
        ...
        { 
            if ( chainsPrefix.length() > 1 ) {
                ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : loadSequencesFromPdb: chains prefix must be empty or exactly one character."<<endl;
                ErrorManager::instance.treatError();
            }
            cout<<__FILE__<<":"<<__LINE__<<endl;
    ...
    if ((parameterStringClass.getString(0)).compare("constrainRigidSegments" ) ==0) {
        ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" This parameter is discontinued.  Use constrainChainRigidSegments instead."<<endl; ErrorManager::instance.treatError();

        /*cout<<__FILE__<<":"<<__LINE__<<" : Syntax: constrainRigidSegments <True | False>"<<endl; 

        parameterStringClass.validateNumFields(2);

        if (removeRigidBodyMomentum) {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You must first set removeRigidBodyMomentum False before constraining anything to ground ! "<<endl;
            ErrorManager::instance.treatError();
        }
    ...
    if ((parameterStringClass.getString(0)).compare( "includeIntraChainInterfaceResidues") == 0) {       
        cout<<__FILE__<<":"<<__LINE__<<" syntax:  includeIntraChainInterfaceResidues <chain> <depth>    "<<endl;         
        cout<<__FILE__<<":"<<__LINE__<<" This will add all inter-BODY interfaces, to a <depth>, in the specified <chain>, to the physics zone.    "<<endl;       
        //cout<<__FILE__<<":"<<__LINE__<<" You can omit <depth>, and it will be assumed that you want to set the depthain>, to the physics zone.    "<<endl;         
        parameterStringClass.validateNumFields(3);       
        ...
        if (myIncludeIntraChainInterface.Depth < 0.0000001) {ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" depth must be >0 !"<<endl; ErrorManager::instance.treatError(); }       
        ...
        for (int i = 0 ; i < includeIntraChainInterfaceVector.size() ; i++) {        
            if (includeIntraChainInterfaceVector[i].Chain.compare(myIncludeIntraChainInterface.Chain ) == 0) {       
                ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" The chain "<< myIncludeIntraChainInterface.Chain <<" is already in the includeIntraChainInterfaceVector! Please do not call this twice!"<<std::endl;  ErrorManager::instance.treatError();       
            }        
        }        
    ...
    if  ((parameterStringClass.getString(0)).compare( "physicsInterfaces") == 0) {
        //exit(1);
        cout<<__FILE__<<":"<<__LINE__<<" Syntax is:  physicsInterfaces <depth (nm)> <chain>  "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" This adds all residues to the physics zone, that are within <depth> of <chain> in all other chains.  It also sets the bond mobility for residues in <chain> that are within <depth> of that other chain -- so the interface is treated symmetrically."<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" You can also specify a multiple-chain set, and find all interfaces between that set and the remainder of chains in the system:"<<endl;
    
        cout<<__FILE__<<":"<<__LINE__<<" Syntax is:  physicsInterfaces <depth (nm)> <chain 1> [<chain 2> [<chain 3> [ ...etc]]]  "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" Lastly, you can specify TWO multiple-chain sets, and find only the interfaces between those two sets.  This is particularly useful if you have chains (e.g. threading templates) in your system which need to be ignored:"<<endl;
    
        cout<<__FILE__<<":"<<__LINE__<<" Syntax is:  physicsInterfaces <depth (nm)> <chain 1> [<chain 2> [<chain 3> [ ...etc]]]  Versus <chain 1> [<chain 2> [<chain 3> [ ...etc]]]  "<<endl;
        if (parameterStringClass.getString(2).length() == 0) {
            cout<<__FILE__<<":"<<__LINE__<<" Not enough parameters for this command! You must specify at least one chain."<<endl; exit(1);
        }
        double depth = myAtoF(userVariables,parameterStringClass.getString(1).c_str());
    ...
    if (( ((parameterStringClass.getString(0)).compare( "includeAllResiduesWithin") == 0)) ||
        ...
        if ((globalCoulombScaleFactor == 0) && (globalVdwScaleFactor == 0)) {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : Please set globalCoulombScaleFactor and/or globalVdwScaleFactor to something other than zero, before issuing this command.  Otherwise, there is no point. You can set safeParameters FALSE to override this."<<endl; ErrorManager::instance.treatError();
        }   
        if ((!(fabs(globalGbsaScaleFactor) <= 0.0000001) ) && (safeParameters)) {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : Please set globalGbsaScaleFactor 0 if you are going to run this command."<<endl; ErrorManager::instance.treatError();
        }

        cout<<__FILE__<<":"<<__LINE__<<" syntax:  includeAllResiduesWithin <radius (nm)> <chain> <residue>    "<<endl;
        cout << __FILE__<<":"<<__LINE__<< " In prior releases of MMB, including 2.10, length was taken in Å -- however we are going back to nm length units, with apologies for the confusion.  "<<endl;
        if (parameterStringClass.getString(2).length() == 0) 
        {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You have not provided enough parameters for this command."<<endl; ErrorManager::instance.treatError();
        }
        ...
        {
            //cout<<__FILE__<<":"<<__LINE__<<" : About to print sequence for chain "<< parameterStringClass.getString(2) <<endl;
    ...
    if ( ((parameterStringClass.getString(0)).compare( "physicsWhereYouWantIt") == 0)) {
        ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : This parameter is obsolete!  You do not need to set it explicitly anymore.  It is set implicitly when you issue any command to add atoms to the force field.  For example, includeAllResiduesWithin or includeAllNonBondAtomsInResidues will do this."<<endl; ErrorManager::instance.treatError();
        parameterStringClass.validateNumFields(2);
    ...
    if (((parameterStringClass.getString(0)).compare("addHuntCrossleySpheres") ==0)||((parameterStringClass.getString(0)).compare("addLargeHuntCrossleySpheres") ==0) ||((parameterStringClass.getString(0)).compare("addHardSphere") ==0) ) {
        parameterStringClass.validateNumFields(2);
        ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : The commands addHuntCrossleySpheres, addLargeHuntCrossleySpheres, and addHardSphere are no longer supported."<<endl;
        ErrorManager::instance.treatError();
        return;
    }
    ...
    if ((parameterStringClass.getString(0)).compare("setForceAndStericScrubber") ==0) {
        ...
        ErrorManager::instance <<__FILE__<<":"<<__LINE__<<": This is no longer a user configurable parameter.  setForceAndStericScrubber is set to TRUE automatically by setting dutyCycle to anythin other than 1.  "<<endl; ErrorManager::instance.treatError();
        parameterStringClass.validateNumFields(2);
        ...
        if ( setForceAndStericScrubber  &&
            ...
            // cout<<__FILE__<<":"<<__LINE__<<": You can't use fitToDensity if you are also going to issue setForceAndStericScrubber TRUE "<<parameterFileName<<endl;
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<": You can't use fitToDensity if you are also going to issue setForceAndStericScrubber TRUE "<<endl;
            ErrorManager::instance.treatError();
        }
        return;
    }
    if ((parameterStringClass.getString(0)).compare("setForceScrubber") ==0) {
        parameterStringClass.validateNumFields(2);
        //cout<<__FILE__<<":"<<__LINE__<<": setForceScrubber is no longer a supported parameter "<<parameterFileName<<endl;
        ErrorManager::instance <<__FILE__<<":"<<__LINE__<<": setForceScrubber is no longer a supported parameter "<<endl;
        ErrorManager::instance.treatError();
    ...
    if ((parameterStringClass.getString(0)).compare("leontisWesthofInFileName") ==0) {
        if (safeParameters) {
	    ErrorManager::instance <<__FILE__<<":"<<__LINE__<<": leontisWesthofInFileName is not a supported parameter at the moment. Please just have a Leontis-Westhof parameter file called parameters.csv in your working directory. "<<endl;
	    ErrorManager::instance.treatError();
        }
        parameterStringClass.validateNumFields(2);
        leontisWesthofInFileName = parameterStringClass.getString(1);
        if (verbose) cout<<__FILE__<<":"<<__LINE__<<" leontisWesthofInFileName ="<<leontisWesthofInFileName<<endl;
        return;
    }
    if ((parameterStringClass.getString(0)).compare("rigidifyTermini") ==0) {
        parameterStringClass.validateNumFields(2);
        // cout<<__FILE__<<":"<<__LINE__<<": rigidifyTermini is no longer a supported parameter "<<parameterFileName<<endl;
        ErrorManager::instance <<__FILE__<<":"<<__LINE__<<": rigidifyTermini is no longer a supported parameter "<<endl;
        ErrorManager::instance.treatError();
    ...
    if ((parameterStringClass.getString(0)).compare("matchingMinimizerTolerance") ==0) {
        //ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : This parameter is obsolete!"<<endl; ErrorManager::instance.treatError();
        parameterStringClass.validateNumFields(2);
    ...
    if ((parameterStringClass.getString(0)).compare("guessCoordinates") ==0) {
        if (safeParameters) {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : This parameter is no longer user configurable.  Use the matchFast, matchGapped, or matchGappedNoHeal macros instead. "<<endl;
            ErrorManager::instance.treatError();
        }
        parameterStringClass.validateNumFields(2);
    ...
    if ((parameterStringClass.getString(0)).compare("matchExact") ==0) {
        ...
        /*if (parameterStringClass.getString(2).length() >0){
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You have provided too many parameters! "<<endl;
            ErrorManager::instance.treatError();
        }*/
        if (! (matchExact || matchIdealized)) {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You must set matchExact and/or matchIdealized to true. Right now both are false. "<<endl;
            ErrorManager::instance.treatError();
        }
    ...
    {
        parameterStringClass.validateNumFields(1);
        cout<<__FILE__<<":"<<__LINE__<<" : This is a macro which sets matchExact to true,  matchIdealized to false.  This will fit to all atoms provided in the input structure file, but will not be able to handle any missing backbone atoms between regions of known structure.  Use this when there are no gaps in your known structure.  missing residues or atoms at the beginning and end of the known region are OK; and excess length will be instantiated as an extended chain.  This is the fastest fitting protocol -- but BEWARE:  you should only do this if you have an MMB-generated double-precision input PDB file (which contains REMARK-SIMTK-COORDS records).  Otherwise the error is likely to accumulate over any chain of nontrivial length."<<endl;
    ...
    {
        parameterStringClass.validateNumFields(1);
        cout<<__FILE__<<":"<<__LINE__<<" : This is a macro which sets matchExact, matchIdealized to true.  This will fit to all atoms provided in the input structure file, and will guess atom positions for all fragments missing from that file, consistent with default bond lengths and angles.  You should generally use this macro when you have atoms missing along the backbone.   "<<endl;
    ...
    {
        //if (safeParameters) {ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : This macro is not currently supported! You should use matchFast, which is every bit as effective and a heck of a lot faster.  That said, matchGapped actually does work, so you can override this by setting safeParameters FALSE "<<endl; ErrorManager::instance.treatError();}
        parameterStringClass.validateNumFields(1);
        cout<<__FILE__<<":"<<__LINE__<<" : This is a macro which sets matchExact, matchIdealized, matchOptimize, and guessCoordinates to true.  This will fit to all atoms provided in the input structure file, and will guess atom positions for all fragments missing from that file, consistent with default bond lengths and angles.  You should generally use this macro when you have atoms missing along the backbone. The missing atoms can be in the middle of a chain, although unnatural bond geometries may occur as MMB does its darnedest to achieve loop closure and match all it can.   "<<endl;
    ...
    {
        if (safeParameters) {ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : This macro is not currently supported!"<<endl; ErrorManager::instance.treatError();}
        cout<<__FILE__<<":"<<__LINE__<<" : This is a macro which sets matchExact to true, and matchIdealized to false.  This will fit to all atoms provided in the input structure file, and will guess atom positions for all fragments missing from that file.  The connection between known and unknown fragments will in general have an unphysical bond geometry.  Use this in unusual cases where physically reasonable connection between fragments of known structure is impossible. Use matchGapped for cases when such a connection IS possible.    "<<endl;
    ...
    if ((parameterStringClass.getString(0)).compare("matchPerfect") ==0) 
    {
        ...
        if (safeParameters) {
        ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : This macro is no longer user configurable.  Use the matchFast, matchGapped, or matchGappedNoHeal macros instead. "<<endl;
        ErrorManager::instance.treatError();}
        cout<<__FILE__<<":"<<__LINE__<<" This is a macro.  If True, it sets BOTH matchExact and matchIdealized to True.  This means that all bond lengths, angles, and dihedrals will be matched locally, and then there will be a global refinement of torsion angles to correct for accumulated error.  This costs as much as matchIdealized, but generally gives better results."<<endl;
        ...
        if (parameterStringClass.getString(1).length() >0){
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You have provided too many parameters! matchPerfect is a macro which takes no parameters."<<endl;
            ErrorManager::instance.treatError();
        }
        return;
    }
    if ((parameterStringClass.getString(0)).compare("minimize") ==0) {
        parameterStringClass.validateNumFields(2);
        //ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" minimize is obsolete."<<endl;
        //ErrorManager::instance.treatError();
    ...
    if ((parameterStringClass.getString(0)).compare("monteCarloRun") ==0) {
        parameterStringClass.validateNumFields(2);
        ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" The monteCarloRun is obsolete."<<endl;
        ErrorManager::instance.treatError();
    ...
    if ((parameterStringClass.getString(0)).compare("") ==0) {
        parameterStringClass.validateNumFields(2);
        if (verbose) cout << __FILE__<<":"<<__LINE__<<" blank line in input file."<<parameterStringClass.getString(0)<<endl;
        return;
    }

    if(parameterStringClass.getString(0).compare("detectConvergence")==0)
    {
        cout << __FILE__<<":"<<__LINE__<<": detectConvergence: stop the stage if the simulation converged, i.e. if the difference in total energy for the few last frames (default 5) is inferior to an epsilon value (default 0.5 kJ/mol)"<< endl;
        parameterStringClass.validateNumFields(2);
...
void ParameterReader::initializeFromFileOnly(const char * parameterFileName ) {
    ...
    if (!(inFile.good())){
        ErrorManager::instance <<__FILE__<<":"<<__LINE__<<": Could not open file "<<parameterFileName<<endl;
        ErrorManager::instance.treatError();
    }
    int readStage = 0;
    ...
    cout<<__FILE__<<":"<<__LINE__<<" : Just set variable @CURRENTSTAGE to "<<userVariables[String("@CURRENTSTAGE")]<<endl;
    ...
    while (inFile.good()) {
        ...
        if (((parameterStringClass.getString(0)).compare("readFromStage") == 0) || ((parameterStringClass.getString(0)).compare("readAtStage") == 0) || ((parameterStringClass.getString(0)).compare("readToStage") == 0) || ((parameterStringClass.getString(0)).compare("readExceptAtStage") == 0)  ) 
        {
            ...
            //cout<<__FILE__<<":"<<__LINE__<<": "<<(parameterStringClass.getString(0)).compare("readAtStage")<<endl;
            if ((parameterStringClass.getString(0)).compare("readAtStage") == 0)
            {
                readAtOneStageOnly = true; 
                cout<<__FILE__<<":"<<__LINE__<<" readAtOneStageOnly: "<<readAtOneStageOnly<<endl;
            }
        ...
        {
            cout<<endl<<__FILE__<<":"<<__LINE__<<" read line: "<<tempString.c_str()<<endl;   
    ...
    if (verbose) cout<<__FILE__<<":"<<__LINE__<<" base pairs just after reading from commands.dat:"<<endl;
    if (verbose) basePairContainer.printBasePairs();
    cout << __FILE__<<":"<<__LINE__<<" After reading command file, here is the list of atom springs: "<<endl;
    atomSpringContainer.printAtomSprings();
    cout << __FILE__<<":"<<__LINE__<<" Done printing atom springs. "<<endl;
...
void ParameterReader::loadSequencesFromPdb(const char * pdbFileName, const string & chainsPrefix, const bool tempRenumberPdbResidues ) { //, vector<std::string> deletedResidueVector ){
    cout<<__FILE__<<":"<<__LINE__<<" : This command will now extract all DNA, RNA, and protein sequences found in the input structure file, "<<pdbFileName<<" and instantiate the corresponding biopolymers. "<<endl;

    // The folowing commented part is not valid since loadSequencesFromPdb takes pdb files as parameters
    // if ((myBiopolymerClassContainer.getNumBiopolymers() >0) ) {
    //     ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : This command may be called only once, and there can be no other biopolymer-instantiating commands preceding it. "<<endl;
    //     ErrorManager::instance.treatError();
    // }
...
void ParameterReader::setFirstAndLastStage(const char * parameterFileName ) {
    ...
    ifstream inFile(parameterFileName,ifstream::in);
    if (! inFile.good()){
        ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" Unable to open command file: "<<parameterFileName<<endl;   
        ErrorManager::instance.treatError();
    }
    ...
    while (inFile.good()) {
        ...
        u.str(tempString); 
        #ifdef _DEBUG_FLAGS_ON_
        cout<<__FILE__<<":"<<__LINE__<<" read line: "<<u.str()<<endl;   
        #endif
        ...
        if  (((mystring[0]).compare("-FS") == 0) || ((mystring[0]).compare("firstStage") == 0))  {
            ...
            if (    position != String::npos) { 
                cout<<__FILE__<<":"<<__LINE__<<" User varibles are not permitted for setting firstStage or lastStage. "<<endl;   
            }
...
void ParameterReader::postInitialize(){
    cout<<__FILE__<<":"<<__LINE__<<" Printing original and renumbered residue numbers: "<<endl;
    myBiopolymerClassContainer.printOriginalAndRenumberedResidueIDs();
    cout<<__FILE__<<":"<<__LINE__<<endl;
    cout<<__FILE__<<":"<<__LINE__<<" In ParameterReader::postInitialize, printing out density stretches: "<<endl;
    //densityContainer.printDensityStretches    (); 
    //densityContainer.stuffDensityStretchVector(myBiopolymerClassContainer); 
    densityContainer.printDensityStretches    (); 
    if (safeParameters) {
        ...
        cout<<__FILE__<<":"<<__LINE__<<" RNABuilder will not reorder base pairs (prioritize 1) for multi-chain jobs.  If you have multiple chains make sure to set prioritize 0."<<endl;
        assert(!((prioritize==1) && (myBiopolymerClassContainer.getNumBiopolymers() >1)));
        assert(!((monteCarloRun==1) && (setChiBondMobility == 1)));
    } else cout<<__FILE__<<":"<<__LINE__<<" You have set safeParameters = 0 and are now syntactically and semantically on your own.  May the RNA gods have mercy on you."<<endl;
    //numDutyCycles = dutyCycleArray.size();//d;


    if (verbose) cout<<__FILE__<<":"<<__LINE__<<" check 14.5"<<endl;
    if (verbose) cout<<__FILE__<<":"<<__LINE__<<" check 15"<<endl;
    if (lastStage == 0) {
        ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" For some reason lastStage is set to 0."<<endl;
        ErrorManager::instance.treatError();
    ...
    if (verbose) cout<<__FILE__<<":"<<__LINE__<<endl;
}
...
void ParameterReader::initializeDefaults(const char * leontisWesthofInFileName){
    ...
    if (!inFile.good()) {
        inFile.close();
        ErrorManager::instance <<__FILE__<<":"<<__LINE__<<": Failed to open leontisWesthofInFileName "<<leontisWesthofInFileName<<endl;
        ErrorManager::instance.treatError();
    }
    inFile.close();
...
void ParameterReader::initialize(const char * parameterFileName ) {
    ifstream inFile(parameterFileName,ifstream::in);
    if (!inFile.good()) {
        ErrorManager::instance <<__FILE__<<":"<<__LINE__<<": Failed to open user input file "<<parameterFileName<<endl;
        ErrorManager::instance.treatError();
    }
